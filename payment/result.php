...
if($paymentId){
    ...
    if($payment){
        ...
        if($payment['type_id']=='1') {
            ...
            $stmt1 = $mysqli->prepare("SELECT id,provider, pnr, travellers FROM bookings_flights WHERE booking_id=? LIMIT 1");
            ...
            if(($fbooking['provider'] ?? '') === "TRAVELPORT"){
                ...
                try{
                    ...
                    // $chargedDecimal = number_format($payment['amount']/100,2,'','');
                    $chargedDecimal = $payment['amount']/100;

                    $commitResult = $api->completeTicketing(
                        $fbooking['pnr'],
                        $cardData,
                        $chargedDecimal,        // use correct amount
                        $payment['currency'],      // use correct currency
                        $booking_id
                    );
                    // print_r($commitResult);

                    sleep(2); // allow ticketing

                    $ticketListResult = $api->getTicketList($fbooking['pnr'],$booking_id);
                    $ticketData = $ticketListResult['data'] ?? [];
                    $ticketNumbers = [];

                    // print_r($ticketListResult);
                    // print_r($booking_id);
                    ...
                    if(!empty($ticketNumbers)){
                        ...
                        $stmt3->bind_param("si",$bookingStatus,$booking_id);
                        $stmt3->execute();

                        $paxList = [];
                        if(!empty($fbooking['travellers'])){
                            $decoded = json_decode($fbooking['travellers'], true);
                            if(json_last_error() === JSON_ERROR_NONE && is_array($decoded)){
                                $paxList = $decoded;
                            }
                        }
                        // loop tickets
                        foreach ($ticketNumbers as $tIdx => $tktNum) {
                            // fallback passenger index
                            // $paxIndex = isset($paxList[$tIdx]) ? $tIdx : 0;
                            // $paxId = $paxList[$paxIndex]['id'] ?? ($paxIndex + 1);
                            $paxId = $tIdx;
                            $tStmt = $mysqli->prepare("
                                INSERT INTO travelport_tickets (booking_id, passenger_id, ticket_number, status) 
                                VALUES (?, ?, ?, 'issued')
                                ON DUPLICATE KEY UPDATE status='issued'
                            ");
                            $tStmt->bind_param("iis", $booking_id, $paxId, $tktNum);
                            $tStmt->execute();
                            $tStmt->close();
                        }
