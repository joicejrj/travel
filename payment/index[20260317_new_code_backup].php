...
// 1. Keep your existing logic for manual entry
onCardBinChanged: async function(_self, cardMetadata) {
    console.log(cardMetadata.bin);
    console.log(cardMetadata);

    handleCardUpdate(cardMetadata);
    return { continue: true };
},
...
onChange: function(state) {
    ...
    // 1. Check if we have card metadata in the state
    // The SDK usually attaches metadata to the state when a card is selected
    // console.log("card_metadata: "+state.card_metadata);
    // console.log(state);  

    if (state && state.card_metadata) {
        handleCardUpdate(state.card_metadata);
    } 
    // 2. If metadata is missing but we have a bin, it's a manual entry
    else if (state.bin && state.bin.length >= 8) {
        // This acts as a backup to onCardBinChanged
    }
},

// 2. ADD THIS: Handle switching between saved cards
// onPaymentMethodChanged: async function(_self, paymentMethod) {
//     // paymentMethod contains metadata for saved cards (stored_card)
//     console.log("onPaymentMethodChanged triggered "+paymentMethod.type);
//     console.log(paymentMethod);
//     if (paymentMethod.type === 'stored_card' && paymentMethod.card_metadata) {
//         handleCardUpdate(paymentMethod.card_metadata);
//     } else {
//         // If they switch back to a non-card method (if applicable), 
//         // reset the fee
//         handleCardUpdate({}); 
//     }
//     return { continue: true };
...
/**
 * Consolidate the fee logic into one reusable function
 */
async function handleCardUpdate(cardMetadata) {
    isCorporateCard = (cardMetadata.card_category || "").toLowerCase() === "commercial";
    ...
    console.log("Updating payment. Category: " + (cardMetadata.card_category || "N/A"));
