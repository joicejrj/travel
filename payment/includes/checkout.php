...
function getSessionByReference(string $reference): ?array {
    $db   = getDB();
    $stmt = $db->prepare("SELECT * FROM bookings_payments WHERE reference = :ref LIMIT 1");
...
function updateSessionStatusById(string $paymentId): ?array {
    ...
    $stmt->execute([
        ...
        ':method'       => $method,
        ':raw_response' => json_encode($details),
        ':session_id'   => $details['metadata']['cko_payment_session_id'] ?? '',
    ]);
...
function getSessionByPaymentId(string $paymentId): ?array {
    $db   = getDB();
    $stmt = $db->prepare("SELECT * FROM bookings_payments WHERE payment_id = :pid LIMIT 1");
...
function getCustomerIdByEmail(string $email): ?string {
    ...
    try {
        ...
        $stmt = $mysqli->prepare("
            SELECT cko_customer_id
            FROM bookings_payments_checkout_customers
            WHERE email = ?
            LIMIT 1
        ");
...
function upsertCustomer(string $email, string $ckoCustomerId): void {
    ...
    try {
        ...
        $stmt = $mysqli->prepare("
            INSERT INTO bookings_payments_checkout_customers
            (email, cko_customer_id)
            VALUES (?, ?)
