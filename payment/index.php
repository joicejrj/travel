...
if($type_id == 1){
    ...
    $stmt = $mysqli->prepare("
    SELECT order_json, provider, total_amount as amount
    FROM bookings_flights
    WHERE booking_id = ?
    LIMIT 1
    ");
    ...
    if($provider === "AMADEUS"){
        ...
        $resv  = $flightData['ReservationResponse']['Reservation'];
        $offer = $resv['Offer'][0];
        ...
        foreach($offers as $offer){
            ...
            foreach($products as $product){
                ...
                if(isset($flightSegments['Flight'])){
                ...
                foreach($flightSegments as $seg){
                    ...
                    $segments[] = [
                        ...
                        'number' => $seg['Flight']['number'],
                        'departure' => [
                            'iataCode' => $seg['Flight']['Departure']['location'],
                            'at' => $seg['Flight']['Departure']['date'].' '.$seg['Flight']['Departure']['time']
                        ],
                        'arrival' => [
                            'iataCode' => $seg['Flight']['Arrival']['location'],
                            'at' => $seg['Flight']['Arrival']['date'].' '.$seg['Flight']['Arrival']['time']
                        ],
        ...
        usort($segments, fn($a,$b)=>$a['sequence'] <=> $b['sequence']);

        // PRICE
        $price = [
            'currency' => $offer['Price']['CurrencyCode']['value'],
            'base'     => $offer['Price']['Base'],
            'total'    => $offer['Price']['TotalPrice']
        ];

        $taxTotal = (float)$offer['Price']['TotalTaxes'];

        // TRAVELLERS
        $travellerCount = count($resv['Traveler']);

        // PNR
        $pnr = $resv['Receipt'][1]['Confirmation']['Locator']['value']
            ?? $resv['Receipt'][0]['Confirmation']['Locator']['value']
            ?? '';
    }
...
if($rowp = $res->fetch_assoc()){
    $corporate_card_fee = (float)$rowp['value'];
}
...
function renderPriceSummary($data){
    ...
    $taxNames = [
    'GB'=>'UK Air Passenger Duty','UB'=>'UK Passenger Service Charge','YQ'=>'Carrier Surcharge',
    'YR'=>'Carrier Surcharge','F6'=>'Passenger Facility Charge','ZR'=>'International Surcharge',
    'OW'=>'Security Charge','XA'=>'APHIS Fee','XY'=>'Immigration Fee','US'=>'US Transportation Tax',
    'AY'=>'Sept 11 Security Fee','XF'=>'Passenger Facility Charge','IN'=>'India GST',
    'WO'=>'India Passenger Service Fee','K3'=>'India User Dev Fee','P2'=>'India Aviation Security Fee',
    // fallback unknowns
    'N4'=>'Airport Tax',
    'W6'=>'Passenger Service Charge',
    'GZ'=>'Government Tax',
    'L2'=>'Airport Development Fee',
    'YX'=>'Airline Surcharge',
    ];
    ...
    $extra = $data['extra'] ?? [];
    $individualTaxes = $extra['individualTaxes'] ?? [];
    $surcharges = $extra['surcharges'] ?? 0;
    ...
    if(!empty($individualTaxes)){

        arsort($individualTaxes);

        $topTaxes  = array_slice($individualTaxes, 0, 2, true);
        $moreTaxes = array_slice($individualTaxes, 2, null, true);

        $collapseId = "taxBox_".uniqid();

        echo '<div class="tax-box">';

        // HEADER
        $totalTax = array_sum($individualTaxes);
        $totalTax = number_format($totalTax,2);

        echo <<<HTML
        <div class="tax-header" data-bs-toggle="collapse" data-bs-target="#{$collapseId}">
            <div>
        ...
        // BODY
        echo "<div class='collapse show tax-body' id='{$collapseId}'>";

        foreach($topTaxes as $code => $val){
        ...
        // MORE (collapsed inside same block)
        if(!empty($moreTaxes)){
            $innerId = $collapseId."_inner";
            $count = count($moreTaxes);

            // HIDDEN EXTRA TAXES (comes right after visible rows)
            echo "<div class='collapse' id='{$innerId}'>";

            foreach($moreTaxes as $code => $val){
                $val = number_format($val,2);
                $name = $taxNames[$code] ?? '';
            ...
            // TOGGLE AT VERY BOTTOM (separate line)
            echo <<<HTML
            <div class="tax-more-toggle text-center mt-2"
             data-bs-toggle="collapse"
             data-bs-target="#{$innerId}"
             onclick="this.innerText = this.innerText.includes('more') ? 'Hide taxes' : '+{$count} more taxes'">
             +{$count} more taxes
            </div>
            HTML;
        }
    ...
    if($surcharges > 0){
        $surcharges = number_format($surcharges,2);
        echo <<<HTML
        <div class="price-row">
...
.tax-header[aria-expanded="true"] .toggle-icon{
            ...
            <?php
                ...
                if($provider === "AMADEUS"){
                    ...
                    foreach($offer['travelerPricings'] as $tp){
                        ...
                        $qty = 1; // each travelerPricing = 1 passenger

                        if(!empty($tp['price']['taxes'])){
                            foreach($tp['price']['taxes'] as $tax){

                                $code  = $tax['code'] ?? '';
                                $value = (float)($tax['amount'] ?? 0);

                                $taxTotal += $value;

                                if(!isset($individualTaxes[$code])){
                                    $individualTaxes[$code] = 0;
                                }

                                $individualTaxes[$code] += $value * $qty;
                            }
                        }
                    }
                    ...
                    $extra = [
                        ...
                        "surcharges"      => 0 // Amadeus usually includes this inside taxes (YQ/YR)
                    ];
                }

                if($provider === "TRAVELPORT" && !empty($flightData)){

                    $resv  = $flightData['ReservationResponse']['Reservation'];
                    $offer = $resv['Offer'][0];

                    $breakdown = $offer['Price']['PriceBreakdown'] ?? [];
                    ...
                    foreach ($breakdown as $pb) {
                        ...
                        // TAXES
                        if(isset($pb['Amount']['Taxes']['Tax'])){
                            foreach ($pb['Amount']['Taxes']['Tax'] as $tax) {
                                $code  = $tax['taxCode'] ?? '';
                        ...
                        // SURCHARGES
                        if(isset($pb['Surcharges']['TotalSurcharges'])){
                            $totalSurcharges += (float)$pb['Surcharges']['TotalSurcharges'] * $qty;
                        }
                    }
...
let backendTotal = <?= (float)$row['amount'] ?>;
let baseAmountMinor = <?= $amount ?>;
let currentAmountMinor = baseAmountMinor;
...
createSession();
    ...
    $("#totalAmount").html(
        backendTotal.toFixed(2)+' <span class="currency">'+cur+'</span>'
    );
});
...
function updateFeeDisplay(isCorporate, cur) {
    const symbol = currSymbols[cur] || cur;
    let base = backendTotal;   // ✅ FIX: use backend total
    let total = base;
    if(isCorporate){
        const fee = base * (card_fee/100);
        total = base + fee;
