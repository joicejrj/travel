<?php
// public/settings.php
// Manage application settings: view + update value per row.
    ...
    if (empty($_POST['csrf_token']) || !hash_equals($csrf, $_POST['csrf_token'])) {
        ...
        if ($action === 'update') {
            ...
            if ($setting_id <= 0) {
                ...
                // Example: you may want to restrict certain settings to numeric values.
                // Leave generic so it accepts strings/numbers. Add validation here if needed.
                $stmt = $mysqli->prepare("UPDATE settings SET value = ?, updated_at = NOW() WHERE id = ?");
                if (!$stmt) {
...
if ($edit_id) {
    $stmt = $mysqli->prepare("SELECT id, name, value, updated_at FROM settings WHERE id=? LIMIT 1");
    if ($stmt) {
...
// Fetch all settings
$list = [];
$res = $mysqli->query("SELECT id, name, value, updated_at FROM settings ORDER BY id ASC");
if ($res) {
...
<div class="container">
  <div class="d-flex justify-content-between align-items-center mb-3">
    <h1 class="h3 mb-0">Configure daily routine</h1>
  </div>
  ...
  <div class="row">
    <div class="col-12">
      <div class="table-responsive">
        <table id="settingsTable" class="table table-sm table-striped" style="width:100%">
          <thead>
            <tr>
              <th>ID</th>
              <th>Name</th>
          ...
          <tbody>
            ...
            <?php endforeach; ?>
            <?php if (empty($list)): ?>
              <tr><td colspan="5" class="text-muted">No settings found.</td></tr>
            <?php endif; ?>
          </tbody>
        </table>
      </div>
