...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['srand']) && isset($_SESSION['srand']) && $_SESSION['srand'] == $_POST['srand']) {
    ...
    if (empty($errors)) {
        ...
        if (!empty($_FILES['photo1']['name'])) {
            $imgt1 = $_FILES['photo1'];
            $img1 = $site->upload_img($imgt1,'uploads/suppliers','random',800,"auto","");
            if($img1!='') $imagePath1 = $img1;
        }
        $imagePath2 = null;
        if (!empty($_FILES['photo2']['name'])) {
            $imgt2 = $_FILES['photo2'];
            $img2 = $site->upload_img($imgt2,'uploads/suppliers','random',800,"auto","");
        ...
        if ($stmt) {
            ...
            $ok = $stmt->execute();
            $new_supplier_id = $mysqli->insert_id;
            $stmt->close();

            if ($ok) {
                $site->agent_log("Added new Supplier ".$name);

                // $db->insert('suppliers_sites',array('supplier_id'=>$new_supplier_id, 'site_name'=>'Site 1', 'site_contact'=>'', 'site_address'=>'', 'site_location'=>'', 'created_by'=>$datetime));
                // $db->insert('suppliers_sites',array('supplier_id'=>$new_supplier_id, 'site_name'=>'Site 2', 'site_contact'=>'', 'site_address'=>'', 'site_location'=>'', 'created_by'=>$datetime));
                // $site->agent_log("Added Site 1 and Site 2 for the supplier automatically");

                // If Add With Reminder: insert reminder row
                if ($add_with_reminder) {
                    // normalize datetime
                    $datetime1 = $reminder_date . ' ' . $reminder_time . ':00';
                    $rstmt = $mysqli->prepare("INSERT INTO suppliers_reminders (supplier_id, type, reminder_at, note, created_at) VALUES (?, ?, ?, ?, NOW())");
                    if ($rstmt) {
                        $default_type = 'Callback';
                        $rstmt->bind_param("isss", $new_supplier_id, $default_type, $datetime1, $notes);
                        $rok = $rstmt->execute();
                        $rstmt->close();
                        if (!$rok) {
                            $errors[] = "Supplier added but failed to add reminder: " . htmlspecialchars($mysqli->error);
                        }
                        else {
                          $site->agent_log("Added a reminder for new Supplier ".$name,$new_supplier_id,"supplier");
                        }
                    } else {
                        $errors[] = "Supplier added but failed to add reminder (DB error): " . htmlspecialchars($mysqli->error);
                    }
                }

                // If no errors so far, redirect to suppliers list
                if (empty($errors)) {
                ...
                $errors[] = "Failed to add Supplier: " . htmlspecialchars($mysqli->error);
            }
        } else {
  ...
  <form method="post" enctype="multipart/form-data" class="card shadow-sm p-4">
    ...
    <div class="mb-3">
      ...
      <div class="d-flex flex-wrap gap-2">
        <?php
        foreach ($suppliers_statuses as $s) {
