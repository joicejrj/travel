...
<div class="container1">
  <div class="d-flex align-items-center mb-3">
    <h1 class="h4 me-3">Templates Manager</h1>
    <div class="ms-auto">
  ...
  <div class="modal-dialog modal-xl modal-dialog-centered" style="max-width:95%;">
    <div class="modal-content">
      ...
      <div class="modal-body p-0">
        <iframe id="pdfFrame" style="width:100%;height:85vh;border:0;"></iframe>
      </div>
    </div>
  </div>
</div>
...
(function(){
  ...
  async function loadEntityData(identifier) {
      ...
      const url = `public/ajax/document_templates.php?action=load_entity
  ...
  async function loadEntityList(type, keyword = "") {
      const url = `public/ajax/document_templates.php?action=search_entities
  ...
  async function loadTemplates(){
    const res = await fetch('public/ajax/document_templates.php?action=list_templates').then(r=>r.json());
    ...
    if (!rows.length) {
        templatesList.innerHTML = `
            <div class="text-muted small p-2">
                No templates found. Please contact admin.
            </div>`;
        return;
    }
    ...
    //     const res = await fetch('public/ajax/document_templates.php?action=delete_template',{method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body:'id='+encodeURIComponent(btn.dataset.id)}).then(r=>r.json());
  ...
  async function openTemplateDetails(id){
    const res = await fetch('public/ajax/document_templates.php?action=list_versions&template_id='+encodeURIComponent(id)).then(r=>r.json());
    ...
    versions.forEach(v=>{
      ...
      box.innerHTML = `
        ...
        <div class="ms-auto text-end">
          ...
          <button class="btn btn-sm btn-success btn-create-doc ms-1" data-id="${v.id}" title="Create Document using this version"><i class="fa fa-file-circle-plus"></i></button>
        </div>
      `;
    ...
    versionsList.querySelectorAll('.btn-preview').forEach(b=>{
      b.onclick = async ()=>{
        const res = await fetch('public/ajax/document_templates.php?action=get_version&id='+encodeURIComponent(b.dataset.id)).then(r=>r.json());
        if(res.ok) {
    ...
    versionsList.querySelectorAll('.btn-set-active').forEach(b=>{
      b.onclick = async ()=>{
        ...
        const res = await fetch('public/ajax/document_templates.php?action=set_active_version', {method:'POST', body}).then(r=>r.json());
    ...
    versionsList.querySelectorAll('.btn-delete-version').forEach(b=>{
      b.onclick = async ()=>{
        ...
        const res = await fetch('public/ajax/document_templates.php?action=delete_version', {method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body:'id='+encodeURIComponent(b.dataset.id)}).then(r=>r.json());
        if(res.ok) openTemplateDetails(b.dataset.template);
    ...
    versionsList.querySelectorAll('.btn-create-doc').forEach(b=>{
      b.onclick = async ()=>{
        ...
        const res = await fetch('public/ajax/document_templates.php?action=get_placeholders&version_id=' + encodeURIComponent(b.dataset.id)).then(r=>r.json());
  ...
  function buildPlaceholderFields(placeholders){
    ...
    if (document.getElementById('ph_date')) {
        let date = new Date();
        let day = String(date.getDate()).padStart(2, '0');
        let month = String(date.getMonth() + 1).padStart(2, '0'); // getMonth() is 0-based
        let year = date.getFullYear();
        document.getElementById('ph_date').value = `${day}-${month}-${year}`;
    }
  ...
  document.getElementById('formTemplate').addEventListener('submit', async function(e){
    ...
    const res = await fetch('public/ajax/document_templates.php?action='+action, {method:'POST', body: fd}).then(r=>r.json());
    if(res.ok) {
      modalTemplate.hide();
      loadTemplates();
    } else alert(res.error || 'Error');
  ...
  document.getElementById('btnCopyActive').onclick = async ()=>{
    ...
    const res = await fetch('public/ajax/document_templates.php?action=list_versions&template_id='+encodeURIComponent(tid)).then(r=>r.json());
  ...
  document.getElementById('formCreateDoc').addEventListener('submit', async function(e){
    ...
    const res = await fetch('public/ajax/document_templates.php?action=create_document', {method:'POST', body}).then(r=>r.json());
    if(res.ok) {
      modalCreateDoc.hide();
      loadDocuments();
  ...
  async function loadDocuments(){
    const res = await fetch('public/ajax/document_templates.php?action=list_documents').then(r=>r.json());
    ...
    documentsList.querySelectorAll('.btn-view-doc').forEach(b=>{
      b.onclick = async ()=>{
        const res = await fetch('public/ajax/document_templates.php?action=get_document&id='+encodeURIComponent(b.dataset.id)).then(r=>r.json());
        if(res.ok) {
          document.getElementById('previewContent').innerHTML = (res.data.content || "").replace(/\n/g, "<br>");
          modalPreview.show();
        } else alert('Error');
    ...
    //     window.open('public/ajax/document_templates.php?action=download_pdf&id=' + encodeURIComponent(id), '_blank');
    //   };
    // });
    documentsList.querySelectorAll('.btn-download-doc').forEach(b=>{
      b.onclick = async ()=>{
        ...
        const pdfUrl = 'public/ajax/document_templates.php?action=download_pdf&id=' + encodeURIComponent(did);

        // const response = await fetch(
        //   'public/ajax/document_templates.php?action=pdf&id=' + id,
        //   { method: "GET" }
        // );
    ...
    documentsList.querySelectorAll('.btn-delete-doc').forEach(b=>{
      b.onclick = async ()=>{
        ...
        const res = await fetch('public/ajax/document_templates.php?action=delete_document', {method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body:'id='+encodeURIComponent(b.dataset.id)}).then(r=>r.json());
    ...
    documentsList.querySelectorAll('.btn-add-profile').forEach(b => {
        b.onclick = async () => {
            ...
            const res = await fetch(
                "public/ajax/document_templates.php?action=add_to_profile",
                { method: "POST", body }
            ).then(r => r.json());
