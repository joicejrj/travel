...
$name = $company  = $phone = $email = $enable_email = $enable_whatsapp = $photo = $photo1 = $agent_id = '';

$is_edit = false; //($id > 0);

// If editing, fetch existing supplier
if ($id > 0) {
    if ($stmt = $mysqli->prepare("SELECT id, name, company, phone, email, enable_email, enable_whatsapp, photo, photo1, agent_id FROM contacts WHERE id = ?")) { // and agent_id= ?
...
<div class="header border-bottom pb-2 mb-3x">
  <div class="d-flex flex-wrap flex-md-nowrap justify-content-between align-items-start gap-2">
    ...
    <div class="d-flex flex-wrap align-items-center gap-2 flex-grow-1">
      ...
      <button class="btn btn-outline-primary active btn-sm tab-btn fw-bold" id="notesBtn" data-tab="notes">Notes</button>
    ...
    <div class="content container-fluid">
      <div class="row h-100 jrow">
        ...
        <div class="col-md-8 col-section jleft">
            ...
            <!-- Notes Tab Content -->
            <div id="notes-section" class="tab-section">
                    ...
                    <div class="modal-body py-2 px-3">
                      ...
                      <div class="mb-3">
                        ...
                        <div class="btn-group w-100 mb-2" id="dateButtons">
                          ...
                          <button type="button" class="btn btn-outline-primary btn-xs date-btn" data-value="<?=date('Y-m-d', strtotime('+1 day'))?>">Tomorrow</button>
                      ...
                      <div class="mb-3">
                        <label class="form-label small fw-semibold">Category
            ...
            <?php
              $getp = $db->get('customers_payments',array('customer_id'=>$customer_id,'#cus'=>"(payment_status!='Paid')"),'sum(invoice_amount) as pamt'); //,'type'=>'Income'
              $pamt = $getp&&$getp->pamt!=''?$getp->pamt:0;
            ?>
              <!-- Payments Section -->
              <div id="payments-section" class="tab-section" style="display:none;">
                  <div class="card shadow-sm mt-3">
                    <div class="card-header d-flex justify-content-between align-items-center bg-white">
                      <h6 class="fw-semibold mb-0">
                        <i class="fa fa-file-payment me-2 text-primary"></i> Payments <b>(Due: <?=$currency_symbol?><?=$pamt?>)</b>
                      </h6>
                    ...
                    <div class="card-body p-2 section-scroll" id="payment-list" style="max-height: 480px;">
                          ...
                          <div class="col-md-4">
                            <label for="payment_type" class="form-label fw-semibold small mb-1 d-flex align-items-center">
                              ...
                              <i class="fa fa-info-circle ms-1 text-primary"
                                 ...
                                 title="Income = Money RECEIVED from employee.&#10;Expense = Money SPENT for employee."></i>
                            </label>
          ...
          <form id="info-wrapper" class="row g-3 text-black">
            ...
            <div class="col-md-4 settinginp settinginpImg">
              ...
              <div id="photo1Preview" class="mt-2">
                <?php if (!empty($photo)): ?>
                  <img src="uploads/contacts/<?= htmlspecialchars($photo) ?>" alt="Photo 1"
            ...
            <div class="col-md-4 settinginp settinginpImg">
              ...
              <div id="photo2Preview" class="mt-2">
                <?php if (!empty($photo1)): ?>
                  <img src="uploads/contacts/<?= htmlspecialchars($photo1) ?>" alt="Photo 2"
                       class="img-thumbnail" style="max-width: 100px;">
                <?php endif; ?>
              </div>
            </div>
...
$(function() {
  ...
  function attachSiteButtons() {
    ...
    $('.delete-site-btn').off('click').on('click', function(){
      ...
      if (site && site.id) {
        ...
        $.post('public/ajax/customers_sites.php', { action:'delete', id: site.id }, function(resp){
          try { 
  ...
  window.openSitesModal = function(customerId) {
    // if you pass customerId it will load for that id; else use page-level var
...
$(function() {
  ...
  $("#supplier-note-form").on("submit", function(e){
    ...
    $.ajax({
      url: "public/ajax/contacts_logs.php",
      type: "POST",
      dataType: "json",
      data: { 
        action: "save",
  ...
  function loadNotes(ref = false) {
    ...
    $.post("public/ajax/contacts_logs.php", 
  ...
  $(document).on("click", ".edit-contact", function(){
    ...
    $.ajax({
      ...
      success: function(data){
        ...
        let cardFront = data.photo1 ? `<img src="uploads/contacts/${data.photo1}" class="img-thumbnail" style="max-width:100px;">` : "";
        let cardBack  = data.photo2  ? `<img src="uploads/contacts/${data.photo2}" class="img-thumbnail" style="max-width:100px;">` : "";
...
function viewContactCard(contactId) {
  ...
  $.ajax({
    ...
    success: function(data) {
      if (data) {
        const side1 = data.photo1 ? `uploads/contacts/${data.photo1}` : "";
        const side2 = data.photo2 ? `uploads/contacts/${data.photo2}` : "";
...
$(function() {
  const customerId = <?= $customer_id ?? 0 ?>;
  const logURL = "public/ajax/contacts_recent_actions.php";
  ...
  function loadLogs() {
    ...
    $.ajax({
      ...
      type: "POST",
      dataType: "json",
      data: { contact_id: customerId, start: 0, length: 10, draw: 1 },
      success: function(res) {
...
$(function () {
      ...
      res.data.forEach(inv => {
        ...
        const docBtn = inv.document
          ? `<button class='btn btn-light border view-document'
                     data-file='uploads/contacts/invoices/${inv.document}'
  ...
  function loadCategories(type, selected = null, cb = null) {
    $.post("public/ajax/invoices_categories.php",
  ...
  $(document).on("click", ".edit-invoice", function () {
      ...
      $.post(ajaxURL, { action: "fetch", customer_id: customerId }, function (res) {
          ...
          else if (invDate === tomorrow) {
              $(".date-btn[data-value='" + tomorrow + "']")
                  .removeClass("btn-outline-primary")
                  .addClass("btn-primary text-white");
              $("#invoice_date_other").addClass("d-none");
              $("#invoice_date").val(tomorrow);
          }
          else {
  ...
  window.generateAutoNotesinvoice = function () {
      ...
      const text = `Invoice for AED ${amount} - ${category}.`;
