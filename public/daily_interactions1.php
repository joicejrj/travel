<?php
// daily_interactions.php  — full updated page (version: ver1.2 + modal fixes)
...
$scenarios = [];
$res = $mysqli->query("SELECT id, name FROM scenarios ORDER BY name");
...
.ee-employee-item:hover { background:#fbfdff; }
.ee-employee-item.selected { background:#eef7ff; border-color:#cfefff; }
  ...
  <aside class="right-panel">
    ...
    <form id="detailForm" data-current-id="" novalidate>
      ...
      <div class="wizard-step" id="wiz-step-2" aria-hidden="true" style="display:none;">
        <div class="line">
          <div class="field-group"><label>Scenario</label>
        ...
        <div class="line" id="relatedEntityWrap" style="display:none;">
          <div class="field-group">
        ...
        <div class="line"><div class="field-group"><label for="detail-subject">Short Summary</label><input type="text" id="detail-subject" name="subject" placeholder="Short summary"></div></div>
        ...
        <div style="display:flex;gap:8px;margin-top:12px;justify-content:flex-end">
          ...
          <button type="button" class="btn-primary" id="wiz-next-2" style="min-width:120px">Next →</button>
        </div>
      </div>
...
<div id="existingEmployeeModalBackdrop" class="ee-modal-backdrop" aria-hidden="true">
  <div id="existingEmployeeModal" class="ee-modal" role="dialog" aria-modal="true" aria-labelledby="ee-modal-title">
    <!-- LEFT: employees -->
    <div class="ee-left">
      <div style="display:flex;align-items:center;justify-content:space-between;">
        <strong id="ee-modal-title">Select Existing Employee</strong>
      ...
      <div class="ee-search-row">
        <input id="ee-search-input" class="ee-search-input" placeholder="Search employees (name / email / id)..." />
        <!-- <button id="ee-search-btn" class="ee-search-btn btn-secondary ee-btn">Search</button> -->
      </div>

      <div id="ee-employee-list" class="ee-employee-list" tabindex="0">
        <div class="muted">Loading…</div>
      </div>
    </div>
    ...
    <div class="ee-right">
      <!-- <h3>Scenarios (employee)</h3> -->
      <div style="display:flex;justify-content:space-between;align-items:center">
        <div>
          <h4 id="empContactsTitle" style="margin:0 0 6px 0">Scenarios</h4>
          <div id="empSelectedLabel" style="font-size:13px;color:var(--muted)">Pick an employee scenario</div>
        </div>
        <div>
      ...
      <div class="ee-footer">
        <input type="hidden" id="ee-selected-employee-id" name="ee_selected_employee_id" value="" />
        <input type="hidden" id="ee-selected-scenario-id" name="ee_selected_scenario_id" value="" />
        <input type="hidden" id="ee-selected-employee-name" value="">
<input type="hidden" id="ee-selected-employee-phone" value="">
<input type="hidden" id="ee-selected-employee-email" value="">
...
$('#confirmTagBtn').on('click', function () {
  ...
  // Inject into form if not already present
  // if ($('#detailForm input[name="nature"]').length === 0) {
  //   $('#detailForm').append(
  //     $('<input>', {
  //       type: 'hidden',
  //       name: 'nature',
...
//     if (!eid) {
//       alert('Please pick an existing entity (customer / employee / recruiter / supplier) before saving.');
//       return;
//     }
//   }
...
//     id: id || null,
//     date: qs('#detail-date') ? qs('#detail-date').value : '',
//     time: qs('#detail-time') ? qs('#detail-time').value : '',
//     contact_name: qs('#detail-contact-name') ? qs('#detail-contact-name').value : '',
//     subject: qs('#detail-subject') ? qs('#detail-subject').value : '',
//     notes: qs('#detail-notes') ? qs('#detail-notes').value : '',
//     channel_id: getActiveDataId('channelGroup'),
//     contact_type_id: getActiveDataId('contactTypeGroup'),
//     scenario_id: (qs('#detail-scenario-id') && qs('#detail-scenario-id').value) 
//              || (qs('#scenario_id') && qs('#scenario_id').value) 
//              || getActiveDataId('scenarioGroup'),
//     owner_id: getActiveDataId('ownerGroup'),
//     status: getActiveDataValue('statusGroup'),
//     priority: getActiveDataValue('priorityGroup'),
//     follow_date: qs('#detail-follow-date') ? qs('#detail-follow-date').value : '',
//     follow_time: qs('#detail-follow-time') ? qs('#detail-follow-time').value : '',
//     contact_entity_type: qs('#detail-contact-entity-type') ? qs('#detail-contact-entity-type').value : '',
//     contact_entity_id: qs('#detail-contact-entity-id') ? qs('#detail-contact-entity-id').value : '',
//     contact_phone: qs('#detail-contact-phone') ? qs('#detail-contact-phone').value : '',
//     contact_email: qs('#detail-contact-email') ? qs('#detail-contact-email').value : '',
//     customer_id: qs('#detail-customer-id') ? qs('#detail-customer-id').value : '',
//     customer_contact_id: qs('#detail-customer-contact-id') ? qs('#detail-customer-contact-id').value : '',
//     related_employee_ids: qs('#related_employee_ids') ? qs('#related_employee_ids').value : '',
//     related_customer_id: qs('#related_customer_id') ? qs('#related_customer_id').value : '',
//   };
//   fetch('public/ajax/ajax_save_interaction.php', {
//     method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(payload)
//   }).then(r=>r.json()).then(res=>{ 
//     if (res && res.success) { alert('Saved'); //location.reload(); 
//   } else alert('Save failed: ' + (res && res.error ? res.error : 'unknown'));
...
function saveDetail(){
  ...
  fd.append(
    'scenario_id',
    qs('#detail-scenario-id')?.value ||
    qs('#scenario_id')?.value ||
    getActiveDataId('scenarioGroup')
  );
  ...
  fetch('public/ajax/ajax_create_interaction.php', { //ajax_save_interaction.php
    method: 'POST',
...
function initBtnGroup(groupId){
  ...
  group.addEventListener('click', function(e){
    ...
    if (groupId === 'contactTypeGroup') {
      ...
      if (/new/i.test(txt) && /contact/i.test(txt)) {
        console.log('[debug] selected -> New Contact');
      ...
      // Existing Recruiters -> recruiters picker
      if (/existing\s+recruiters?/i.test(txt)) {
        qsa('#' + groupId + ' .btn-as-select').forEach(b=>b.classList.remove('active'));
        btn.classList.add('active');
        const hid = group.querySelector('input[type="hidden"]');
        if (hid) hid.value = btn.dataset.id || btn.dataset.value || '';
        ...
        showManualContact(false);
        clearPickedEntityUI();
        setInterTypeForModal('recruiter');
        openRecruitersPicker();
        return;
      }
      ...
      // Existing Customer -> open the (original) customer picker and show related-employee option
      if (/existing\s+customer/i.test(txt)) {
        ...
        showManualContact(false);
        clearPickedEntityUI();
        setInterTypeForModal('customer');
        // open the customer picker and enable related-employee UI
        if (typeof openCustomerPicker === 'function') {
          try { openCustomerPicker(); } catch(err) { console.error('openCustomerPicker error', err); }
        }
        if (typeof showRelatedEntityOptionForType === 'function') {
          try { showRelatedEntityOptionForType('customer'); } catch(err){ /* ignore */ }
        }
        return;
      }

      // Existing Employee -> existing employee flow (keeps previous behavior)
      if (/existing\s+employee/i.test(txt)) {
        ...
        if (step1Visible) {
          ...
          if (typeof window.ee_openExistingEmployeeModal === 'function') {
            try {
              window.ee_openExistingEmployeeModal();
              // do not call legacy pickers in Step1
              return;
            } catch(err) {
              console.error('ee_openExistingEmployeeModal error', err);
...
function openEntityPickerFallback(entityType) {
  ...
  fetch('?page=daily_interactions&list_entities=' + encodeURIComponent(entityType))
...
(function($){
  ...
  var $backdrop = $('#existingEmployeeModalBackdrop');
  var $modal = $('#existingEmployeeModal');
  var $searchInput = $('#ee-search-input');
  var $searchBtn = $('#ee-search-btn');
  var $employeeList = $('#ee-employee-list');
  ...
  var $selectedEmployeeId = $('#ee-selected-employee-id');
  var $selectedScenarioId = $('#ee-selected-scenario-id');

  var ee_state = { employee: null, scenario: null };
  ...
  window.ee_openExistingEmployeeModal = function() {
    // reset selections
  ...
  function ee_fetchEmployees(q, limit) {
    limit = limit || 50;
    $employeeList.html('<div class="muted">Loading…</div>');
    var url = 'public/ajax/ajax_list_employees.php?limit=' + encodeURIComponent(limit);
    if (q && q.trim() !== '') url += '&q=' + encodeURIComponent(q.trim());
    $.getJSON(url).done(function(res){
      if (!res || !res.success) { $employeeList.html('<div class="muted">Failed to load employees.</div>'); return; }
      var items = res.items || [];
      if (items.length === 0) { $employeeList.html('<div class="muted">No employees found.</div>'); return; }
      var html = '';
items.forEach(function(it){
    // Define variables FIRST
    var displayName = it.name || ('#' + it.id);
    var meta       = it.email ? it.email : (it.phone ? it.phone : '');
    var empId      = it.emp_id || ''; // backend now returns this

    html += '<div class="ee-employee-item"'
         +   ' data-ee-id="' + it.id + '"'
...
$employeeList.find('.ee-employee-item').off('click').on('click', function(){
    $employeeList.find('.ee-employee-item.selected').removeClass('selected');
    ...
    }).fail(function(){ $employeeList.html('<div class="muted">Failed to load employees.</div>'); });
  }
  ...
  function ee_setEmployee(emp) {
    ...
    $('#ee-selected-employee-id').val(emp.id);
    $('#ee-selected-employee-name').val(emp.name || '');
    $('#ee-selected-employee-phone').val(emp.phone || '');
    $('#ee-selected-employee-email').val(emp.email || '');

    if ($('#ee-selected-employee-empid').length === 0) {
    $('<input>').attr({ type: 'hidden', id: 'ee-selected-employee-empid', name: 'ee_selected_employee_empid' }).appendTo($('form').first() || $('body'));
  }
  $('#ee-selected-employee-empid').val(emp.emp_id || '');
  ...
  function ee_loadScenariosOnce(cb) {
    ...
    $scenarioList.html('<div class="muted">Loading scenarios…</div>');
    $.getJSON('public/ajax/ajax_list_scenarios.php?inter_type=employee&limit=500').done(function(res){
  ...
  function renderScenarioList(list) {
    ...
    if (!items.length) { $scenarioList.html('<div class="muted">No scenarios found. Add one below.</div>'); return; }
    var html = '';
  ...
  if (!ee_state.employee || !ee_state.scenario) { alert('Select an employee and a scenario.'); return; }
...
(function(){
  ...
  function ee_applyScenarioIfEmployee() {
    ...
    if (isEmployee && hasScenario) {
        ...
        ro.innerHTML =
            '<strong>Selected Scenario</strong><br>' +
