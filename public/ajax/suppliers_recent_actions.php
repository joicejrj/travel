...
if ($type === 'timeline') {
    ...
    $sql = "
        ...
        WHERE l.supplier_id = ? and l.type!='notimeline'
        ORDER BY l.timestamp DESC
        LIMIT ?, ?
    ";

    $stmt = $mysqli->prepare($sql);
    $stmt->bind_param("iii", $supplier_id, $start, $length);
    $stmt->execute();
...
if ($searchValue !== '') {
  ...
  $countStmt->bind_param("isss", $supplier_id, $likeSearch, $likeSearch, $likeSearch);
} else {
...
if ($searchValue !== '') {
  $dataStmt->bind_param("isssii", $supplier_id, $likeSearch, $likeSearch, $likeSearch, $start, $length);
} else {
  $dataStmt->bind_param("iii", $supplier_id, $start, $length);
}
