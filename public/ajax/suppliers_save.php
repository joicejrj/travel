...
if ($supplier_id > 0 && $field == 'photo' && isset($_FILES['photo']) && $_FILES['photo']['error'] == 0) {
    ...
    if (in_array($ext, $allowedExts) && $file['size'] <= 10*1024*1024) { // max 10MB (as previously)
        $newName = $site->upload_img($file,'../uploads/suppliers','random',800);
        if($newName!='') {
            $geti = $db->get('suppliers', array('id' => $supplier_id), 'photo');
            $upd  = $db->update('suppliers', ['id' => $supplier_id], ['photo' => $newName]);
            if ($upd) {
                if ($geti && $geti->photo != '') {
                    $site->remove_file('../../uploads/suppliers/' . $geti->photo);
                }
                // $getcontact = $db->get('suppliers',array('id'=>$supplier_id),'name');
                $site->agent_log("Supplier Address ID 1 is updated",$supplier_id,"supplier");
...
// === Handle photo1 upload ===
    ...
    if (in_array($ext, $allowedExts) && $file['size'] <= 10*1024*1024) {
        $newName = $site->upload_img($file,'../uploads/suppliers','random',800);
        if($newName!='') {
            $geti = $db->get('suppliers', array('id' => $supplier_id), 'photo1');
            $upd  = $db->update('suppliers', ['id' => $supplier_id], ['photo1' => $newName]);
            if ($upd) {
                // $getcontact = $db->get('suppliers',array('id'=>$supplier_id),'name');
                $site->agent_log("Supplier Address ID 2 is updated",$supplier_id,"supplier");
                if ($geti && $geti->photo1 != '') {
                    $site->remove_file('../../uploads/suppliers/' . $geti->photo1);
                }
                $response = ['success' => true, 'msg' => 'Photo uploaded successfully', 'photo' => $newName];
            } else {
                $response['msg'] = 'Database update failed';
...
// === Handle normal field updates ===
    ...
    if ($field === 'type') {
        ...
        // get previous status
        $prev = $db->get('suppliers', array('id' => $supplier_id), 'type');
        $prev_type = $prev ? $prev->type : '';

        // Update the contact's status
        $upd = $db->update('suppliers', array('id' => $supplier_id), array('type' => $value));
        if ($upd) {
            // $getcontact = $db->get('suppliers',array('id'=>$supplier_id),'name');
            $site->agent_log("Status of Supplier is updated to ".$value,$supplier_id,"supplier");
            // Log status change to suppliers_logs (audit)
            $logData = [
            ...
            // Try to insert into suppliers_logs or fallback to generic logs table name
            if ( method_exists($db, 'insert') ) {
                // Try suppliers_logs
                $inserted = $db->insert('suppliers_logs', $logData);
                if ($inserted === false) {
                ...
                $stmt = $mysqli->prepare("INSERT INTO suppliers_logs (supplier_id, name, notes, created_at) VALUES (?, ?, ?, ?)");
                if ($stmt) {
    ...
    // Special handling for enable_email and enable_whatsapp toggles
        ...
        // Get previous state
        $prev = $db->get('suppliers', ['id' => $supplier_id], $field);
        ...
        // Update database
        $upd = $db->update('suppliers', ['id' => $supplier_id], [$field => $value]);
        if ($upd) {
            ...
            // Write agent log
            $site->agent_log("{$prettyField} status changed to {$newStatus}",$supplier_id,"supplier");

            // Prepare log data for suppliers_logs
            $logData = [
                'supplier_id' => $supplier_id,
            ...
            // Try to insert log into suppliers_logs table
            if (method_exists($db, 'insert')) {
                $inserted = $db->insert('suppliers_logs', $logData);
                if ($inserted === false) {
                    // fallback to contact_logs if suppliers_logs not found
                    @$db->insert('contact_logs', $logData);
                }
            } else {
        ...
        // Normal update path
        $getcontact = $db->get('suppliers', ['id' => $supplier_id], "name, $field");
        ...
        $upd = $db->update('suppliers', array('id' => $supplier_id), array($field => $value));
        if ($upd) {

            // update existing contact number to suppliers phones json array
            $valuex = preg_replace('/\D/', '', $value);
            if (($field == 'phone' || $field == 'whatsapp') && strlen($valuex) >= 8) {
                $gets = $db->get('suppliers', array('id' => $supplier_id), 'phones');
                if ($gets) {
                    ...
                    if (!empty($phones)) {
                        ...
                        $db->update('suppliers', ['id' => $supplier_id], $mainc);
                    }
                }
            }

            if ($field == 'email' && $value != '') {
                $db->update('suppliers', ['id' => $supplier_id], array('fil_emails' => $value));
            }

            $site->agent_log("Supplier ".$field." is updated to ".$value,$supplier_id,"supplier");
