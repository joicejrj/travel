...
if ($action === 'fetch') {
    ...
    if ($result->num_rows === 0) {
        echo '<p class="text-muted text-center small">No contacts found for this recruiter.</p>';
        exit;
    }
            ...
            <div class='d-flex justify-content-between align-items-start'>
              <div class='flex-grow-1'>
                <p class='fw-semibold mb-1 text-dark'>
                  $name " . (!empty($designation) ? "<small class='text-muted'>($designation)</small>" : "") . "
                </p>
...
if ($action === 'save') {
    ...
    $uploadDir = __DIR__ . '/../../uploads/recruiters/contacts/';
    if (!is_dir($uploadDir)) {
    ...
    if ($id) {
        ...
        $existing = $mysqli->query("SELECT photo1, photo2, name FROM recruiters_contacts WHERE id=" . intval($id))->fetch_assoc();
        ...
        // 📝 Update contact
        $stmt = $mysqli->prepare("
            UPDATE recruiters_contacts 
            SET recruiter_id=?, name=?, phone=?, email=?, designation=?, photo1=?, photo2=? 
            WHERE id=?");
        $stmt->bind_param("issssssi", $recruiter_id, $name, $phone, $email, $designation, $photo1, $photo2, $id);

        $succe = $stmt->execute();
        if($succe) {
            $site->agent_log("Updated contact ".$existing['name']."[$id] ($phone $email $designation)",$recruiter_id,'recruiter');
        }
        ...
        // 🆕 Add new contact
        $stmt = $mysqli->prepare("
            INSERT INTO recruiters_contacts (recruiter_id, name, phone, email, designation, photo1, photo2, created_by)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("isssssss", $recruiter_id, $name, $phone, $email, $designation, $photo1, $photo2, $created_by);

        $succe = $stmt->execute();
        if($succe) {
            $site->agent_log("Added new contact with ".$name." ($phone $email $designation)",$recruiter_id,'recruiter');
        }
        echo $succe ? 'success' : 'error';
...
if ($action === 'get') {
    $id = intval($_POST['id']);
    $stmt = $mysqli->prepare("SELECT * FROM recruiters_contacts WHERE id = ?");
...
if ($action === 'delete') {
    ...
    $stmt1 = $mysqli->prepare("SELECT * FROM recruiters_contacts WHERE id=?");
    $stmt1->bind_param("i", $id);
    $stmt1->execute();
    $res = $stmt1->get_result()->fetch_assoc();
    $stmt1->close();

    $stmt = $mysqli->prepare("DELETE FROM recruiters_contacts WHERE id = ?");
    $stmt->bind_param("i", $id);

    $succe = $stmt->execute();
    if($succe) {
        $site->agent_log("Deleted contact ".$res['name']."[".$res['id']."]",$recruiter_id,'recruiter');
    }
    echo $succe ? 'success' : 'error';
...
if ($action === 'list') {
    ...
    $stmt = $mysqli->prepare("SELECT id, name FROM recruiters_contacts WHERE recruiter_id = ? ORDER BY name ASC");
    $stmt->bind_param("i", $recruiter_id);
    $stmt->execute();
    $result = $stmt->get_result();
