...
if ($action === 'fetch') {
    ...
    if ($result->num_rows === 0) {
        echo '<p class="text-muted text-center small">No contacts found for this supplier.</p>';
        exit;
    }
...
if ($action === 'save') {
    ...
    $uploadDir = __DIR__ . '/../../uploads/suppliers/contacts/';
    if (!is_dir($uploadDir)) {
    ...
    if ($id) {
        ...
        $existing = $mysqli->query("SELECT photo1, photo2, name FROM suppliers_contacts WHERE id=" . intval($id))->fetch_assoc();
        ...
        $stmt = $mysqli->prepare("
            ...
            SET supplier_id=?, name=?, phone=?, email=?, designation=?, photo1=?, photo2=? 
            WHERE id=?");
        $stmt->bind_param("issssssi", $supplier_id, $name, $phone, $email, $designation, $photo1, $photo2, $id);

        $succe = $stmt->execute();
        if($succe) {
            $site->agent_log("Updated contact ".$existing['name']."[$id] ($phone $email $designation)",$supplier_id,"supplier");
        }
        ...
        // 🆕 Add new contact
        $stmt = $mysqli->prepare("
            INSERT INTO suppliers_contacts (supplier_id, name, phone, email, designation, photo1, photo2, created_by)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("isssssss", $supplier_id, $name, $phone, $email, $designation, $photo1, $photo2, $created_by);

        $succe = $stmt->execute();
        if($succe) {
            $site->agent_log("Added new contact with ".$name." ($phone $email $designation)",$supplier_id,"supplier");
        }
        echo $succe ? 'success' : 'error';
...
if ($action === 'get') {
    $id = intval($_POST['id']);
    $stmt = $mysqli->prepare("SELECT * FROM suppliers_contacts WHERE id = ?");
...
if ($action === 'delete') {
    ...
    $stmt1 = $mysqli->prepare("SELECT * FROM suppliers_contacts WHERE id=?");
    $stmt1->bind_param("i", $id);
    $stmt1->execute();
    $res = $stmt1->get_result()->fetch_assoc();
    $stmt1->close();

    $stmt = $mysqli->prepare("DELETE FROM suppliers_contacts WHERE id = ?");
    $stmt->bind_param("i", $id);

    $succe = $stmt->execute();
    if($succe) {
        $site->agent_log("Deleted contact ".$res['name']."[".$res['id']."]",$supplier_id,"supplier");
    }
    echo $succe ? 'success' : 'error';
...
if ($action === 'list') {
    ...
    $stmt = $mysqli->prepare("SELECT id, name FROM suppliers_contacts WHERE supplier_id = ? ORDER BY name ASC");
    $stmt->bind_param("i", $supplier_id);
    $stmt->execute();
    $result = $stmt->get_result();
