...
$type = $_POST['type'] ?? '';   // ⭐ NEW: type=timeline
...
if ($type === 'timeline') {
    ...
    $sql = "
        ...
        WHERE l.recruiter_id = ? and l.type!='notimeline'
        ORDER BY l.timestamp DESC
        LIMIT ?, ?
    ";

    $stmt = $mysqli->prepare($sql);
    $stmt->bind_param("iii", $recruiter_id, $start, $length);
    $stmt->execute();
...
$columns = ['l.id', 'l.log', 'l.agent_id', 'l.timestamp'];
...
if ($searchValue !== '') {
  ...
  $countStmt->bind_param("isss", $recruiter_id, $likeSearch, $likeSearch, $likeSearch);
} else {
  $countStmt->bind_param("i", $recruiter_id);
}
...
if ($searchValue !== '') {
  $dataStmt->bind_param("isssii", $recruiter_id, $likeSearch, $likeSearch, $likeSearch, $start, $length);
} else {
  $dataStmt->bind_param("iii", $recruiter_id, $start, $length);
}
