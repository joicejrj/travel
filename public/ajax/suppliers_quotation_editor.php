...
if ($action == 'load_editor') {
    ...
    /* -------------------------------------------------------
       1️⃣  LOAD JOB REQUIREMENTS FROM suppliers_requirements
    ------------------------------------------------------- */
    $q = $mysqli->prepare("SELECT * FROM suppliers_requirements WHERE supplier_id = ? ORDER BY id ASC");
    $q->bind_param("i", $supplier_id);
    $q->execute();
    $result = $q->get_result();
    $index = 1;
    ...
    if ($result->num_rows > 0) {
        ...
        <div class='alert alert-light border text-center small mb-0'>
          No job requirements found for this supplier.<br>
