...
$uploadDir = __DIR__ . '/../../uploads/suppliers/documents/';
...
if ($action === 'fetch') {
    ...
    $stmt = $mysqli->prepare("SELECT * FROM suppliers_documents WHERE supplier_id=? ORDER BY id DESC");
    ...
    while ($r = $result->fetch_assoc()) {
        ...
        $fileLink = "./uploads/suppliers/documents/" . htmlspecialchars($r['file_name']);
...
if ($action === 'save') {
    ...
    $stmt = $mysqli->prepare("
        INSERT INTO suppliers_documents (supplier_id, label, file_name, file_type, expiry_date, created_by)
        VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("isssss", $supplier_id, $label, $fileName, $file_type, $expiry_date, $created_by);

    $succe = $stmt->execute();
    if($succe) {
        $site->agent_log("Added new Document ".$label." (".$fileName.")",$supplier_id,"supplier");
        ...
        if ($expiry_date && strtotime($expiry_date)) {
          ...
          if ($reminder_at && strtotime($reminder_at)) {
              $db->insert('suppliers_reminders',array('supplier_id'=>$supplier_id,'type'=>'General','reminder_at'=>$reminder_at,'note'=>"Document $label expiry on ".date("d M Y",strtotime($expiry_date)),'created_at'=>$datetime));
          }
        }
...
if ($action === 'delete') {
    ...
    $stmt = $mysqli->prepare("SELECT file_name,label FROM suppliers_documents WHERE id=?");
    ...
    $stmt = $mysqli->prepare("DELETE FROM suppliers_documents WHERE id=?");
    $stmt->bind_param("i", $id);

    $succe = $stmt->execute();
    if($succe) {
        $site->agent_log("Deleted Document ".$res['label']." (".$res['file_name'].")",$supplier_id,"supplier");
    }
    echo $succe ? 'success' : 'error';
