...
if ($type === 'timeline') {
    ...
    $sql = "
        ...
        WHERE l.customer_id = ? and l.type!='notimeline'
        ORDER BY l.timestamp DESC
        LIMIT ?, ?
    ";

    $stmt = $mysqli->prepare($sql);
    $stmt->bind_param("iii", $customer_id, $start, $length);
    $stmt->execute();
...
if ($searchValue !== '') {
  ...
  $countStmt->bind_param("isss", $customer_id, $likeSearch, $likeSearch, $likeSearch);
} else {
...
if ($searchValue !== '') {
  $dataStmt->bind_param("isssii", $customer_id, $likeSearch, $likeSearch, $likeSearch, $start, $length);
} else {
  $dataStmt->bind_param("iii", $customer_id, $start, $length);
}
