...
$sql = "
  SELECT r.id, r.{$rem_col} AS contact_ref, r.reminder_at, r.type, r.note, r.completed,
         c.id AS customer_id, c.name AS contact_name, c.email AS contact_email, c.phone AS contact_phone, 
         c.agent_id AS contact_agent, c.type AS contact_status, r.contact_id
  FROM {$use_table} r
  LEFT JOIN customers c ON r.{$rem_col} = c.id
