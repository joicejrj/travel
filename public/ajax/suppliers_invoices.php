...
$uploadDir = __DIR__ . '/../../uploads/suppliers/invoices/';
...
switch ($action) {
  ...
  case 'fetch':
    ...
    // FINAL QUERY
    $res = $mysqli->query("SELECT * FROM suppliers_invoices WHERE $where ORDER BY id DESC");
    $data = [];
  ...
  case 'save':
    ...
    if ($id > 0) {
        ...
        $old = $mysqli->query("SELECT document FROM suppliers_invoices WHERE id=$id")->fetch_assoc();
        ...
        $stmt = $mysqli->prepare("
            ...
            (supplier_id, invoice_date, due_date, invoice_amount, type, category, document, notes, created_by)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ");
    ...
    if ($ok) {
        ...
        $site->agent_log($text, $supplier_id, 'supplier');
    }
  ...
  case 'delete':
    $id = intval($_POST['id'] ?? 0);
    $old = $mysqli->query("SELECT document,supplier_id FROM suppliers_invoices WHERE id=$id")->fetch_assoc();
    if (!empty($old['document']) && file_exists($uploadDir . $old['document'])) unlink($uploadDir . $old['document']);
    $mysqli->query("DELETE FROM suppliers_invoices WHERE id=$id");
    $site->agent_log("Invoice [$id] deleted",$old['supplier_id'],'supplier');
