...
if ($action === 'list_old') {
    ...
    $stmt = $mysqli->prepare("
        SELECT * FROM suppliers_quotations 
        WHERE supplier_id = ? 
...
if ($action === 'list') {
    ...
    $stmt = $mysqli->prepare("
        SELECT q1.*, 
               (SELECT COUNT(*) 
                FROM suppliers_quotations qx 
                WHERE qx.supplier_id = q1.supplier_id 
                  AND qx.ref_no = q1.ref_no) AS version_count
        FROM suppliers_quotations q1
        INNER JOIN (
            SELECT ref_no, MAX(version) AS max_ver
            FROM suppliers_quotations
            WHERE supplier_id = ?
            GROUP BY ref_no
        ) q2 
        ON q1.ref_no = q2.ref_no AND q1.version = q2.max_ver
        WHERE q1.supplier_id = ?
...
if ($action === 'get_versions') {
    ...
    $stmt = $mysqli->prepare("
        SELECT * FROM suppliers_quotations
        WHERE supplier_id = ? AND ref_no = ?
        ORDER BY version DESC
    ");
    $stmt->bind_param("is", $supplier_id, $ref_no);
    $stmt->execute();
    $res = $stmt->get_result();
...
if ($action === 'save') {
    ...
    $qextra = "";
    if(!$is_new) {
        $qextra = " and ref_no in (select ref_no from suppliers_quotations WHERE id=$q_id)";
    }

    /* --- Get next version number --- */
    $ver_stmt = $mysqli->prepare("SELECT MAX(version) AS max_ver,ref_no FROM suppliers_quotations WHERE supplier_id=?".$qextra);
    $ver_stmt->bind_param("i", $supplier_id);
    $ver_stmt->execute();
    ...
    if (!empty($ver_res['ref_no']) && !$is_new) {
        ...
        $today = date('ymd');
        $ref_no = 'QTN' . $today . $supplier_id . str_pad(rand(1, 999), 3, '0', STR_PAD_LEFT);
        $next_version = 1;
    }

    /* --- Insert quotation --- */
    $stmt = $mysqli->prepare("
        INSERT INTO suppliers_quotations
        (supplier_id, ref_no, version, quotation_name, quotation_date, attention, subject, message, jobs_json, terms_json, closing, created_by)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ");
    $stmt->bind_param(
        "isisssssssss",
    ...
    $succe = $stmt->execute();
    if($succe) {
        $site->agent_log("Quotation ref #$ref_no (v$next_version) created",$supplier_id,"supplier");
    }
...
if ($action === 'get') {
    $id = intval($_POST['id']);
    $stmt = $mysqli->prepare("SELECT * FROM suppliers_quotations WHERE id=?");
...
if ($action === 'clone') {
    $id = intval($_POST['id']);
    $res = $mysqli->query("SELECT * FROM suppliers_quotations WHERE id=$id");
    if (!$res->num_rows) {
    ...
    $ver_stmt = $mysqli->prepare("SELECT MAX(version) AS max_ver FROM suppliers_quotations WHERE supplier_id=?");
    $ver_stmt->bind_param("i", $r['supplier_id']);
    $ver_stmt->execute();
    $ver_res = $ver_stmt->get_result()->fetch_assoc();
    $next_version = intval($ver_res['max_ver'] ?? 0) + 1;
    ...
    $stmt = $mysqli->prepare("
        INSERT INTO suppliers_quotations
        (supplier_id, ref_no, version, quotation_date, attention, subject, message, jobs_json, terms_json, closing, status, created_by)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft', ?)
    ");
    $stmt->bind_param(
        "isissssss",
