<?php
// public/ajax/ajax_add_supplier_contact.php
...
if (!$supplier_id) { echo json_encode(['success'=>false,'error'=>'missing_supplier_id']); exit; }
if ($name === '') { echo json_encode(['success'=>false,'error'=>'missing_name']); exit; }

$sql = "INSERT INTO `suppliers_contacts` (supplier_id, name, designation, phone, email, created_at)
        VALUES (?, ?, ?, ?, ?, NOW())";
$stmt = $mysqli->prepare($sql);
if (!$stmt) { echo json_encode(['success'=>false,'error'=>'db_prepare_failed','sql_error'=>$mysqli->error]); exit; }
$stmt->bind_param('issss', $supplier_id, $name, $designation, $phone, $email);
if (!$stmt->execute()) {
...
$sql2 = "SELECT id, COALESCE(name,'') AS name, COALESCE(email,'') AS email, COALESCE(phone,'') AS phone, COALESCE(designation,'') AS designation FROM `suppliers_contacts` WHERE id = ? LIMIT 1";
