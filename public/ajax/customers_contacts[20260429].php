            ...
            <div class='d-flex justify-content-between align-items-start'>
              <div class='flex-grow-1'>
                <p class='fw-semibold mb-1 text-dark'>
                  $name " . (!empty($dob) && $dob!="0000-00-00"  ? "<small class='text-muted ms-2'>".(date("d-m-Y",strtotime($dob)))."</small>" : "") . "
                </p>
...
if ($action === 'save') {
    ...
    if ($id) {
        ...
        $stmt = $mysqli->prepare("
            ...
            SET customer_id=?, name=?, phone=?, email=?, designation=?, dob=?, photo1=?, photo2=? 
            WHERE id=?");
        $stmt->bind_param("isssssssi", $customer_id, $name, $phone, $email, $designation, $dob, $photo1, $photo2, $id);

        $succe = $stmt->execute();
        if($succe) {
            $site->agent_log("Updated contact ".$existing['name']."[$id] ($phone $email $dob)",$customer_id);
        }
        ...
        // 🆕 Add new contact
        $stmt = $mysqli->prepare("
            INSERT INTO customers_contacts (customer_id, name, phone, email, designation, dob, photo1, photo2, created_by)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?,?)");
        $stmt->bind_param("issssssss", $customer_id, $name, $phone, $email, $designation, $dob, $photo1, $photo2, $created_by);

        $succe = $stmt->execute();
        if($succe) {
            $site->agent_log("Added new contact with ".$name." ($phone $email $dob)",$customer_id);
        }
        echo $succe ? 'success' : 'error';
