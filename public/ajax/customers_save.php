...
$allowed = ['company','name','phone','whatsapp','email','source','type','industry','trn','address','city','state','country','website','services','google_rating','agent_id','fil_domains','fil_emails','enable_email','enable_whatsapp','dob', 'gender']; // add photo
...
if ($supplier_id > 0 && $field == 'photo' && isset($_FILES['photo']) && $_FILES['photo']['error'] == 0) {
    ...
    if (in_array($ext, $allowedExts) && $file['size'] <= 10*1024*1024) { // max 10MB (as previously)
        $newName = $site->upload_img($file,'../uploads/customers','random',800);
        if($newName!='') {
            $geti = $db->get('customers', array('id' => $supplier_id), 'photo');
            $upd  = $db->update('customers', ['id' => $supplier_id], ['photo' => $newName]);
            if ($upd) {
                if ($geti && $geti->photo != '') {
                    $site->remove_file('../../uploads/customers/' . $geti->photo);
                }
                // $getcontact = $db->get('customers',array('id'=>$supplier_id),'name');
                $site->agent_log("Customer Address ID 1 is updated",$supplier_id);
...
// === Handle photo1 upload ===
    ...
    if (in_array($ext, $allowedExts) && $file['size'] <= 10*1024*1024) {
        $newName = $site->upload_img($file,'../uploads/customers','random',800);
        if($newName!='') {
            $geti = $db->get('customers', array('id' => $supplier_id), 'photo1');
            $upd  = $db->update('customers', ['id' => $supplier_id], ['photo1' => $newName]);
            if ($upd) {
                // $getcontact = $db->get('customers',array('id'=>$supplier_id),'name');
                $site->agent_log("Customer Address ID 2 is updated",$supplier_id);
                if ($geti && $geti->photo1 != '') {
                    $site->remove_file('../../uploads/customers/' . $geti->photo1);
                }
                $response = ['success' => true, 'msg' => 'Photo uploaded successfully', 'photo' => $newName];
            } else {
                $response['msg'] = 'Database update failed';
...
// === Handle normal field updates ===
    ...
    if ($field === 'type') {
        ...
        // get previous status
        $prev = $db->get('customers', array('id' => $supplier_id), 'type');
        $prev_type = $prev ? $prev->type : '';

        // Update the contact's status
        $upd = $db->update('customers', array('id' => $supplier_id), array('type' => $value));
        if ($upd) {
            // $getcontact = $db->get('customers',array('id'=>$supplier_id),'name');
            $site->agent_log("Status of Customer is updated to ".$value,$supplier_id);
            // Log status change to customers_logs (audit)
            $logData = [
            ...
            // Try to insert into customers_logs or fallback to generic logs table name
            if ( method_exists($db, 'insert') ) {
                // Try customers_logs
                $inserted = $db->insert('customers_logs', $logData);
                if ($inserted === false) {
                ...
                $stmt = $mysqli->prepare("INSERT INTO customers_logs (customer_id, name, notes, created_at) VALUES (?, ?, ?, ?)");
                if ($stmt) {
    ...
    // Special handling for enable_email and enable_whatsapp toggles
        ...
        // Get previous state
        $prev = $db->get('customers', ['id' => $supplier_id], $field);
        ...
        // Update database
        $upd = $db->update('customers', ['id' => $supplier_id], [$field => $value]);
        if ($upd) {
            ...
            // Write agent log
            $site->agent_log("{$prettyField} status changed to {$newStatus}",$supplier_id);
        ...
        // Normal update path
        $getcontact = $db->get('customers', ['id' => $supplier_id], "name, $field");
        ...
        $upd = $db->update('customers', array('id' => $supplier_id), array($field => $value));
        if ($upd) {
            ...
            if (($field == 'phone' || $field == 'whatsapp') && strlen($valuex) >= 8) {
                $gets = $db->get('customers', array('id' => $supplier_id), 'phones');
                if ($gets) {
                    ...
                    if (!empty($phones)) {
                        ...
                        $db->update('customers', ['id' => $supplier_id], $mainc);
                    }
                }
            }

            if ($field == 'email' && $value != '') {
                $db->update('customers', ['id' => $supplier_id], array('fil_emails' => $value));
            }

            $site->agent_log("Customer ".$field." is updated to ".$value,$supplier_id);
