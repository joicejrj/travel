...
switch ($action) {
  ...
  /* ==========================================================
     ...
     ========================================================== */
  case 'save':
    $customer_id = intval($_POST['supplier_id'] ?? 0);
    ...
    $stmt = $mysqli->prepare("
      INSERT INTO customers_logs (customer_id, name, notes, agent_id, type, visibility, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    ");
    $stmt->bind_param("ississs", $customer_id, $name, $notes, $agent_id, $type, $visibility, $datetime);
  ...
  case 'update':
    ...
    $stmt = $mysqli->prepare("UPDATE customers_logs SET notes=?, type=?, visibility=? WHERE id=? AND (agent_id=? OR visibility='Public')");
    $stmt->bind_param("sssii", $notes, $type, $visibility, $id, $agent_id);
    $success = $stmt->execute();
    $stmt->close();

    if($success) {
      $site->agent_log("Note updated (Note: {$notes})",$customer_id);
    }
  ...
  case 'delete':
    ...
    $stmt = $mysqli->prepare("DELETE FROM customers_logs WHERE id = ? AND agent_id = ?");
    $site->agent_log("Note deleted [$id]",$customer_id);
