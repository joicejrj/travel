...
/* ---------- Verify ownership: reminder -> contact -> employees.agent_id = current user ---------- */
$stmt = $mysqli->prepare("
  SELECT r.id
  FROM employees_reminders r
  JOIN employees c ON r.employee_id = c.id
  WHERE r.id = ?
  LIMIT 1
");
if (!$stmt) {
...
if ($action === 'update_note') {
    ...
    $u = $mysqli->prepare("UPDATE employees_reminders SET note = ?, updated_at = NOW() WHERE id = ?");
...
if ($action === 'snooze') {
    ...
    $u = $mysqli->prepare("UPDATE employees_reminders SET reminder_at = DATE_ADD(reminder_at, INTERVAL ? MINUTE), note = ?, updated_at = NOW() WHERE id = ?");
    ...
    $q = $mysqli->prepare("SELECT reminder_at FROM employees_reminders WHERE id = ? LIMIT 1");
...
if ($action === 'complete') {
    ...
    $u = $mysqli->prepare("UPDATE employees_reminders SET completed = 1, note = ?, updated_at = NOW() WHERE id = ?");
