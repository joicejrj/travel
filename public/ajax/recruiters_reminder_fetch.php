...
$rno = $db->get('recruiters_reminders',array('recruiter_id'=>$recruiter_id,'#srt'=>'reminder_at desc'),'count(id) as no');
$pgno = intval($rno->no/$limit);
$reminders = $db->get('recruiters_reminders',array('#all'=>1,'recruiter_id'=>$recruiter_id,'#srt'=>'reminder_at desc','#page'=>$page,'#limit'=>10),'id, reminder_at, type, note, contact_id, created_at');
foreach ($reminders->data as $key => $remi) {
    $reminders->data[$key]->contact_name = '';
    if($remi->contact_id!='') {
        $getcontact = $db->get('recruiters_contacts',array('id'=>$remi->contact_id),'id,name');
        if($getcontact) {
