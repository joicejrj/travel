...
$uploadDir = __DIR__ . '/../../uploads/recruiters/invoices/';
...
switch ($action) {
  ...
  case 'fetch':
    ...
    // FINAL QUERY
    $res = $mysqli->query("SELECT * FROM recruiters_invoices WHERE $where ORDER BY id DESC");
    $data = [];
  ...
  case 'save':
    ...
    if ($id > 0) {
        ...
        $old = $mysqli->query("SELECT document FROM recruiters_invoices WHERE id=$id")->fetch_assoc();
        ...
        ------------------------------ */
        $stmt = $mysqli->prepare("
            INSERT INTO recruiters_invoices 
            (recruiter_id, invoice_date, due_date, invoice_amount, type, category, document, notes, created_by)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        ");
    ...
    if ($ok) {
        ...
        $text = "$laction | Date: ".(date("d M Y",strtotime($invoice_date)))." | Amount: $invoice_amount | Type: $invoice_type | Category: $category";
        $site->agent_log($text, $recruiter_id, 'recruiter');
    }
  ...
  case 'delete':
    $id = intval($_POST['id'] ?? 0);
    $old = $mysqli->query("SELECT document,recruiter_id FROM recruiters_invoices WHERE id=$id")->fetch_assoc();
    if (!empty($old['document']) && file_exists($uploadDir . $old['document'])) unlink($uploadDir . $old['document']);
    $mysqli->query("DELETE FROM recruiters_invoices WHERE id=$id");
    $site->agent_log("Invoice [$id] deleted",$old['recruiter_id'],'recruiter');
