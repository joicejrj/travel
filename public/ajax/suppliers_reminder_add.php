...
// Insert into suppliers_reminders
$created_at = date('Y-m-d H:i:s');
$stmt = $mysqli->prepare("INSERT INTO suppliers_reminders (supplier_id, reminder_at, type, contact_id, note, created_at) VALUES (?, ?, ?, ?, ?, ?)");
if (!$stmt) {
    $rtn['msg'] = "Prepare failed: " . $mysqli->error;
    echo json_encode($rtn);
    exit;
}
$stmt->bind_param("isssss", $supplier_id, $reminder_at, $type, $contact_id, $note, $created_at);
...
if (!$success) {
    ...
    $site->agent_log("New Reminder added for ".date("d M Y h:i A",strtotime($reminder_at)),$supplier_id,"supplier");
}
