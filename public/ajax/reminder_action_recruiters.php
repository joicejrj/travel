...
/* ---------- Verify ownership: reminder -> contact -> recruiters.agent_id = current user ---------- */
$stmt = $mysqli->prepare("
  SELECT r.id
  FROM recruiters_reminders r
  JOIN recruiters c ON r.recruiter_id = c.id
  WHERE r.id = ?
  LIMIT 1
");
if (!$stmt) {
...
if ($action === 'update_note') {
    ...
    $u = $mysqli->prepare("UPDATE recruiters_reminders SET note = ?, updated_at = NOW() WHERE id = ?");
...
if ($action === 'snooze') {
    ...
    $u = $mysqli->prepare("UPDATE recruiters_reminders SET reminder_at = DATE_ADD(reminder_at, INTERVAL ? MINUTE), note = ?, updated_at = NOW() WHERE id = ?");
    ...
    $q = $mysqli->prepare("SELECT reminder_at FROM recruiters_reminders WHERE id = ? LIMIT 1");
...
if ($action === 'complete') {
    ...
    $u = $mysqli->prepare("UPDATE recruiters_reminders SET completed = 1, note = ?, updated_at = NOW() WHERE id = ?");
