...
$uploadDir = __DIR__ . '/../../uploads/suppliers/payments/';
...
switch ($action) {
  ...
  /* ============================================================
    ...
    $res = $mysqli->query("SELECT * FROM suppliers_payments WHERE $where ORDER BY id DESC");
                    ...
                    foreach ($decoded as $d) {
                        if (!empty($d['file'])) {
                            $docs[] = [
                                'file' => $d['file'],
                                'name' => $d['name'] ?? '',
                                'url'  => 'uploads/suppliers/payments/' . $d['file'],
  ...
  case 'save':
      ...
      $old_files = [];
      if ($id > 0) {
          $old = $mysqli->query("SELECT document FROM suppliers_payments WHERE id=$id")->fetch_assoc();
      ...
      if ($id > 0) {
          ...
          $stmt = $mysqli->prepare("
              INSERT INTO suppliers_payments 
              (supplier_id, invoice_date, type, category, payment_status, invoice_amount, invoice_partial, invoice_payment_method, reclaim_by, card_last4, cheque_bank, cheque_issuer, reimbursable, reimbursement_amount, document, notes, created_by)
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
          $stmt->bind_param("isssssssssssssssi", $supplier_id, $invoice_date, $invoice_type, $invoice_cat, $payment_status, $invoice_amount, $invoice_partial, $invoice_payment_method, $reclaim_by, $card_last4, $cheque_bank, $cheque_issuer, $reimbursable, $reimbursement_amount, $fileJson, $notes, $agent_id);
  ...
  case 'delete':
    ...
    $mysqli->query("DELETE FROM suppliers_payments WHERE id=$id");
    $site->agent_log("Payment [$id] deleted",$supplier_id,'supplier');
  ...
  case 'get':
    $id = intval($_POST['id'] ?? 0);
    $row = $mysqli->query("SELECT * FROM suppliers_payments WHERE id=$id LIMIT 1")->fetch_assoc();
    if ($row) {
      $docs = [];
      if (!empty($row['document'])) {
        $decoded = json_decode($row['document'], true);
