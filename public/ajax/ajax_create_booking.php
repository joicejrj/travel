...
if($scenario_id=="1") { //Flights
  ...
  if($provider === 'TRAVELPORT') {
    ...
    try {
        ...
        $offer = json_decode($_POST['selected_offer_json'] ?? '', true);
        $selectedFlights = json_decode($_POST['selected_flights'] ?? '', true);
        $selectedOffer   = json_decode($_POST['selected_offer_json'] ?? '', true);

        // $isMultiCity = is_array($selectedFlights) && count($selectedFlights) > 1;
        $isRoundTrip = strtoupper($trip_type) === 'ROUNDTRIP' || strtoupper($trip_type) === 'ROUND_TRIP';
        $isMultiLeg  = is_array($selectedFlights) && count($selectedFlights) > 1;

        if (!$isMultiLeg && !$selectedOffer) {
            throw new Exception('Invalid offer');
        }
        // if(!$offer) throw new Exception('Invalid offer');

        $firstLegOffer = null;

        $bookingFlights = [];
        if ($isMultiLeg) {
            foreach ($selectedFlights as $i => $f) {
                if (empty($f)) continue;
                if ($firstLegOffer === null) $firstLegOffer = $f;
                $bookingFlights[] = [
                    'leg' => $i + 1,
                    'offer' => $f,
                    'payload' => $f['tp_offer_payload'] ?? []
                ];
            }
            $selectedOffer = $firstLegOffer;
        } else {
            $bookingFlights[] = [
                'leg' => 1,
                'offer' => $selectedOffer,
                'payload' => $selectedOffer['tp_offer_payload'] ?? []
            ];
        }

        $travellersInput = $_POST['travellers'] ?? [];
        if(empty($travellersInput)) throw new Exception('No travellers');

        if (($isRoundTrip || $isMultiLeg) && count($bookingFlights) < 2) {
            throw new Exception("Multi-leg trip requires at least 2 flights");
        }

        /* ✅ MUST HAVE PRICE PAYLOAD */
        // $pricePayload = $_POST['tp_offer_payload'] ?? null;
        // if(!$pricePayload) throw new Exception('Missing tp_offer_payload');
        // $pricePayloads = [];
        // foreach ($bookingFlights as $bf) {
        //     if (!empty($bf['payload'])) {
        //         $p = is_array($bf['payload']) ? $bf['payload'] : json_decode($bf['payload'], true);
        //         if ($p) $pricePayloads[] = $p;
        //     }
        // }
        // if (empty($pricePayloads)) {
        //     throw new Exception('Missing tp_offer_payload');
        // }

        // $pricePayload = is_array($pricePayload) ? $pricePayload : json_decode($pricePayload, true);
        // if(!$pricePayload) throw new Exception('Invalid tp_offer_payload');
        ...
        /* ---------------------------------------
           1. WORKBENCH
        --------------------------------------- */
        $wb = $api->initiateWorkbench();
        $reservationId = $api->extractIdentifier($wb['data']);
        if(!$reservationId) throw new Exception('No reservationId');
        ...
        foreach($travellersInput as $t){
          ...
          if(strlen($firstName) < 2 || strlen($lastName) < 2){
              throw new Exception('Invalid traveller name');
          }

            $email = trim($t['email'] ?? '');
            // always initialize on first traveller
            if ($tIdx == 0) {
                $emailP = $email;
            }
            // if later traveller has email and first was empty → update it
            if ($email !== '' && ($emailP ?? '') === '') {
                $emailP = $email;
            }
            // fallback
            ...
            $dob = trim($t['dob'] ?? '');
            if ($name === '' || $email === '' || $dob === '') {
                throw new Exception('Each traveller must have name, dob and email');
            }

            $tIdx++;
            $type = strtoupper($t['type'] ?? 'ADT');
            $ref  = "T".$tIdx;

            $payload = [
                'first_name' => $firstName,
                'last_name'  => $lastName,
                // 'gender'     => strtoupper(substr($t['gender'] ?? 'M',0,1)),
                ...
                'passport_country'=> strtoupper($t['passport_nationality'] ?? '')
            ];

            /* ✅ INF MUST LINK TO ADULT */
            if($type === 'INF' && !empty($adultRefs)){
                $payload['associatedAdultRef'] = $adultRefs[0];
            }

            $tp = $api->buildTravelerPayload($payload, $ref);

            $res = $api->addTraveler($reservationId, $tp);

            $travId = $api->extractIdentifier($res['data']) ?: $ref;
            
            // $api->addSSR($reservationId, $travId, $payload);

            if($type === 'ADT') $adultRefs[] = $travId;
        }


        try {
        // ================================================================
        // Reference Payload Price (recommended by Travelport for GDS + NDC)
        // POST /air/price/offers/buildfromcatalogproductofferings
        // Sends identifiers from search response instead of full flight details
        // ================================================================

        $catalogIdentifier = $_SESSION['catalog_identifier'] ?? '';
        if (!$catalogIdentifier) {
            throw new Exception("Search cache expired. Please search again.");
        }

        // Build CatalogProductOfferingSelection from selected offer(s)
        $selections = [];

        // if (1==2 && $isRoundTrip && $isReturnStep && $outboundOffer && $returnOffer) { // roundtrip
        //     // Round-trip: two selections (outbound + return)
        //     foreach ([$outboundOffer, $returnOffer] as $offer) {
        //         $productIds = [];
        //         foreach (($offer['productRefs'] ?? []) as $pr) {
        //             $productIds[] = ['Identifier' => ['value' => $pr]];
        //         }
        //         // Fallback: extract from rawBrandOffering
        //         if (empty($productIds)) {
        //             $rawProds = $offer['rawBrandOffering']['Product'] ?? [];
        //             if (isset($rawProds['productRef'])) $rawProds = [$rawProds];
        //             foreach ($rawProds as $rp) {
        //                 if (!empty($rp['productRef'])) $productIds[] = ['Identifier' => ['value' => $rp['productRef']]];
        //             }
        //         }
        //         $selections[] = [
        //             'CatalogProductOfferingIdentifier' => [
        //                 'Identifier' => ['value' => $offer['offeringId']],
        //             ],
        //             'ProductIdentifier' => $productIds,
        //         ];
        //     }
        // } else {
        //     // One-way: single selection
        //     $productIds = [];
        //     foreach (($selectedOffer['productRefs'] ?? []) as $pr) {
        //         $productIds[] = ['Identifier' => ['value' => $pr]];
        //     }
        //     if (empty($productIds)) {
        //         $rawProds = $selectedOffer['tp_raw']['rawBrandOffering']['Product'] ?? [];
        //         if (isset($rawProds['productRef'])) $rawProds = [$rawProds];
        //         foreach ($rawProds as $rp) {
        //             if (!empty($rp['productRef'])) $productIds[] = ['Identifier' => ['value' => $rp['productRef']]];
        //         }
        ...
        //     $selections[] = [
        //         'CatalogProductOfferingIdentifier' => [
        //             'Identifier' => ['value' => $selectedOffer['offeringId']],
        //         ],
        //         'ProductIdentifier' => $productIds,
        //     ];
        // }
        ...
        foreach ($bookingFlights as $bf) {
            ...
            // fallback
            if (empty($productIds)) {
                $rawProds = $offer['tp_raw']['rawBrandOffering']['Product'] ?? [];

                if (isset($rawProds['productRef'])) $rawProds = [$rawProds];

                foreach ($rawProds as $rp) {
                    if (!empty($rp['productRef'])) {
                        $productIds[] = [
                            'Identifier' => ['value' => $rp['productRef']]
                        ];
                    }
                }
            }
        ...
        if($counts['ADT'] > 0){
            $pc[] = ['@type'=>'PassengerCriteria','number'=>$counts['ADT'],'passengerTypeCode'=>'ADT'];
        }
        if($counts['CNN'] > 0){
            $pc[] = ['@type'=>'PassengerCriteria','number'=>$counts['CNN'],'passengerTypeCode'=>'CNN','age'=>8];
        }
        if($counts['INF'] > 0){
            $pc[] = ['@type'=>'PassengerCriteria','number'=>$counts['INF'],'passengerTypeCode'=>'INF'];
        }

        $priceResult = $api->priceOfferReference($catalogIdentifier, $selections, $pc ?: null);
        $priceData = $priceResult['data'] ?? null;

        $priceErrors = $api->extractResponseErrors($priceResult['data'] ?? []);
        if (!empty($priceErrors)) {
            throw new Exception($priceErrors[0]['Message'] ?? 'Pricing error');
        }

        // var_dump($catalogIdentifier);
        // print_r($selections);
        // var_dump("-----------------------");
        // print_r($priceResult);
        // die();

        // Extract price details from response
        // $pricedOffer = null;
        // if ($priceData) {
        //     // Try various response paths
        //     $pricedOffer = $priceData['OfferResponse'] ?? $priceData['Offer'] ?? $priceData;
        // }
        // Store for booking step — reference payload identifiers
        // $_SESSION['selected_offer'] = $selectedOffer;
        // $_SESSION['price_response'] = $priceData;
        // $_SESSION['price_result'] = $priceResult;
        // $_SESSION['ref_catalog_identifier'] = $catalogIdentifier;
        // $_SESSION['ref_selections'] = $selections;

        // Extract price identifier from response (for Add Offer reference payload)
        // OfferListResponse/Identifier/value — remove _PC suffix
        $priceIdentifier = $priceData['OfferListResponse']['Identifier']['value']
                        ?? $priceData['Identifier']['value'] ?? '';
        $priceIdentifier = rtrim($priceIdentifier, '_PC');
        $refCatalogId = "";
        $refSelections = "";
        if ($priceIdentifier) {
            $refCatalogId = $priceIdentifier;
        }

        // Extract priced offer ID and product IDs from price response for Add Offer
        $pricedOfferIds = $priceData['OfferListResponse']['OfferID'] ?? [];
        if (isset($pricedOfferIds['@type'])) $pricedOfferIds = [$pricedOfferIds];
        $priceSelections = [];
        foreach ($pricedOfferIds as $poi) {
            $productIds = [];
            $products = $poi['Product'] ?? [];
            if (isset($products['@type'])) $products = [$products];
            foreach ($products as $prod) {
                $pid = $prod['id'] ?? '';
                if ($pid) $productIds[] = ['Identifier' => ['value' => $pid]];
            }
            $priceSelections[] = [
                'CatalogProductOfferingIdentifier' => [
                    'Identifier' => ['value' => $poi['Identifier']['value'] ?? $poi['id'] ?? ''],
                ],
                'ProductIdentifier' => $productIds,
            ];
        }
        if (!empty($priceSelections)) {
            $refSelections = $priceSelections;
        }
        ...
        /* ---------------------------------------
           3. ADD OFFER (CRITICAL FIX)
        --------------------------------------- */

        // if(empty($pricePayload['OfferQueryBuildFromProducts'])){
        //     throw new Exception('Invalid price payload structure');
        // }

        /* ✅ ALWAYS rebuild PassengerCriteria */
        // $counts = ['ADT'=>0,'CNN'=>0,'INF'=>0];
        ...
        /* 🔥 FORCE override */
        // $pricePayload['OfferQueryBuildFromProducts']['BuildFromProductsRequest']['PassengerCriteria'] = $pc;

        /* 🔥 DEBUG (IMPORTANT — check once) */
        // print_r($pricePayload); exit;

        /* ✅ USE MODIFIED PAYLOAD ONLY */
        // $api->addOffer($reservationId, $pricePayload);
        // if ($isMultiCity) {

        //   foreach ($pricePayloads as $i => $pricePayload) {

        //       if(empty($pricePayload['OfferQueryBuildFromProducts'])){
        //           throw new Exception('Invalid price payload structure');
        //       }

        //       // rebuild passenger criteria
        //       // $counts = ['ADT'=>0,'CNN'=>0,'INF'=>0];
        ...
        //       // ✅ ADD EACH LEG SEPARATELY (IMPORTANT)
        //       // $response = $api->addOffer($reservationId, $pricePayload);

        //       // // ✅ error handling per leg
        //       // $errors = $api->extractResponseErrors($response['data'] ?? []);
        //       // if (!empty($errors)) {
        //       //     throw new Exception("Leg ".($i+1)." failed: ".($errors[0]['Message'] ?? 'Offer error'));
        //       // }

        //       // $refCatalogId  = $_SESSION['ref_price_identifier'] ?? '';
        //       // $refSelections = $_SESSION['ref_price_selections'] ?? [];

        //       if (!empty($refCatalogId) && !empty($refSelections)) {

        //           // $api->addOfferReference($reservationId, $refCatalogId, $refSelections, $bookingId);
        //           $response = $api->addOfferReference(
        //               $reservationId,
        //               $refCatalogId,
        //               $refSelections
        //           );
        ...
        //           $errors = $api->extractResponseErrors($response['data'] ?? []);
        //           if (!empty($errors)) {
        //               throw new Exception($errors[0]['Message'] ?? 'Offer error');
        //           }
        ...
        //     // ✅ keep old logic (oneway / roundtrip SAFE)
        ...
        // print_r($pricePayloads[0]);
        // die();

        /* ---------------------------------------
           4. COMMIT (HOLD)
        --------------------------------------- */
        $commit = $api->commitReservation($reservationId);
        $data = $commit['data'];

        $error = $data['ReservationResponse']['Result']['Error'] ?? null;
        if ($error) {
            $msg = is_array($error)
                ? ($error[0]['Message'] ?? 'Unknown Travelport error')
                : $error;

            throw new Exception("Travelport Error: ".$msg);
        }
        ...
        --------------------------------------- */
        $pnr = null;
        if(preg_match('/"value"\s*:\s*"([A-Z0-9]{6})"/', json_encode($data), $m)){
            $pnr = $m[1];
        }
        ...
        // $destination = end($offer['segments'])['destination'];
        // $depDate     = $offer['segments'][0]['departureDate'];
        // $itin = $offer['itineraries'][0] ?? null;
        $firstFlight = $bookingFlights[0]['offer'];
        $itin = $firstFlight['itineraries'][0] ?? null;
        $segs = $itin['segments'] ?? [];
        if(empty($segs)){
            throw new Exception('Invalid itinerary structure');
        }
        // $first = $segs[0];
        // $last  = end($segs);
        // $origin      = $first['departure']['iataCode'] ?? '';
        // $destination = $last['arrival']['iataCode'] ?? '';
        // $depDate     = isset($first['departure']['at']) ? substr($first['departure']['at'],0,10) : '';

        $firstFlight = $bookingFlights[0]['offer'];
        $lastFlight  = end($bookingFlights)['offer'];
        $firstSeg = $firstFlight['itineraries'][0]['segments'][0];
        $lastSegs = $lastFlight['itineraries'][0]['segments'];
        $lastSeg  = end($lastSegs);
        ...
        $currency = 'GBP';
        foreach ($bookingFlights as $bf) {
            $totalPrice = (float)($bf['offer']['price']['grandTotal'] ?? 0); // removed +=
            $currency = $bf['offer']['price']['currency'] ?? $currency;
        }
        ...
        // $trip_type = !empty($offer['return']) ? 'round_trip' : 'one_way';
        $people_no = (int)count($travellersInput);
        ...
        $stmt = $mysqli->prepare("
            ...
            (booking_id, provider, pnr, origin, destination, departure_date, trip_type, people_no, class, total_amount, currency, traveller_name, traveller_email, traveller_phone, order_json, travellers)
            VALUES (NULL,'TRAVELPORT',?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        ");
    ...
    $non_stop = $_POST['non_stop'] ?? 0;
      ...
      if (!empty($offer['itineraries'][1]['segments'])) {
        ...
        foreach ($travellersInput as $t) {
          ...
          if($email!='') {
            $trav['contact']['emailAddress'] = $email;
          }
      ...
      $currency = $offer['price']['currency'] ?? 'GBP';
        ...
        $c_name = $travellerName;
        $c_email = $travellerEmail;
...
/* ================================
  ...
  try {
    ...
    // $tour_price       = $original_amount = (float)($_POST['selected_tour_price'] ?? 0);
    $original_amount = (float)($_POST['original_amount'] ?? 0);
    $tour_price      = $original_amount; // snapshot
    $travellers_count = (int)($_POST['people_no'] ?? 1);
    $travel_date = trim($_POST['travel_date'] ?? null);

    $discount_id_client = (int)($_POST['discount_id'] ?? 0);
    ...
    if ($rooms < 1) {
        throw new Exception('Invalid room count');
    }
    ...
    $travellerEmail = $travellers[0]["email"];
    $travellerName = $travellers[0]["name"];
    $c_email = $travellerEmail!=""?$travellerEmail:$c_email;
    $c_name = $travellerName!=""?$travellerName:$c_name;
    ...
    if ($discount_id_client > 0) {

        $stmt = $mysqli->prepare("
            SELECT
                id, discount_code, discount_name,
                discount_type, discount_value,
                max_discount_amount,
                scope_type, min_amount,
                valid_from, valid_to, expiry_date
            FROM discounts
            WHERE id = ?
              AND status = 'active'
            LIMIT 1
        ");
        $stmt->bind_param('i', $discount_id_client);
        $stmt->execute();
        ...
        if ($d) {
            ...
            switch ($d['scope_type']) {
                ...
                case 'amount_based':
                    $eligible = $original_amount >= (float)$d['min_amount'];
                    break;

                case 'date_and_amount':
                    $eligible =
                        $original_amount >= (float)$d['min_amount'] &&
                        $travel_date >= $d['valid_from'] &&
                        $travel_date <= $d['valid_to'];
                    break;
            }
            ...
            if ($eligible) {
                ...
                if ($d['discount_type'] === 'percentage') {
                    $discount_amount =
                        $original_amount * ($d['discount_value'] / 100);
                } else {
                    $discount_amount = $d['discount_value'];
                }

                if (!empty($d['max_discount_amount'])) {
                    $discount_amount = min(
                        $discount_amount,
                        (float)$d['max_discount_amount']
                    );
                }

                $discount_amount = round(max(0, $discount_amount), 2);
...
$contact_name = $contact_name==""?($c_name!=""?$c_name:"No Name"):$contact_name;
...
$contact_email = $contact_email==""?$c_email:$contact_email;
...
$rurl = 'bookings_view&id='.$booking_id;
$ufolder = "customers/";
$utype = "customer";
    ...
    if ($target === 'contacts') {
        ...
        if ($entity_id) $contact_record_id = $entity_id;
          ...
          $found = false;
            ...
            $s = $mysqli->prepare("SELECT id FROM customers WHERE phone = ? LIMIT 1");
            if ($s) { 
              $s->bind_param('s', $contact_phone); 
              $s->execute(); 
              $s->bind_result($cid); 
              if ($s->fetch()) { 
                $contact_record_id = $cid; 
                $found = true; 
              } 
              $s->close(); 
