...
if ($action) {
    ...
    if ($action === 'list') {
        ...
        // Count total records
        $sqlCount = "SELECT COUNT(*) AS cnt FROM bookings i";
        ...
        $sql = "SELECT i.*, ch.name AS channel_name, ct.name AS contact_type_name, ct.slug as contact_type_slug, ct.edit_url as contact_type_edit_url,
                       sc.name AS type_name, p.name AS owner_name, ass.name as assigned_name
                FROM bookings i
                LEFT JOIN channels ch ON i.channel_id = ch.id
                LEFT JOIN contact_types ct ON i.contact_type_id = ct.id
                LEFT JOIN bookings_types sc ON i.type_id = sc.id
        ...
        while ($r = $res->fetch_assoc()) {
            ...
            $pnr = "";
            if($r['type_id']=="1") {
                $getflight = $db->get('bookings_flights',array('booking_id'=>$r['id']),'pnr');
                $pnr = $getflight?$getflight->pnr:"";
            }
            ...
            $row[] = htmlspecialchars($r['type_name'] ?: '-');
            ...
            // $actions = '<div class="d-flex gap-1"><button class="btn btn-sm btn-outline-primary" onclick="openFollowups(' . $r['id'] . ')"><i class="fa fa-comments"></i></button><button class="btn btn-sm btn-outline-info" onclick="openBookingDocuments('.$r['id'].')" title="View Documents"><i class="fa fa-paperclip"></i></button><button type="button" class="btn btn-sm btn-outline-primary btn-view" data-id="'.(int)$r['id'].'" title="View"><i class="fa fa-eye"></i></button><button type="button" class="btn btn-sm btn-outline-success btn-go" data-contactediturl="'.$editUrlAttr.'" data-contactentityid="'.$entityIdAttr.'" data-id="'.(int)$r['id'].'" title="Open Booking"><i class="fa fa-arrow-up-right-from-square"></i></button><button type="button" class="btn btn-sm btn-outline-warning btn-edit" data-id="'.(int)$r['id'].'" title="Edit"><i class="fa fa-edit"></i></button></div>';
            $actions = '<div class="d-flex gap-1"><button type="button" class="btn btn-sm btn-outline-primary btn-view" data-id="'.(int)$r['id'].'" data-contactediturl="'.$editurl.'" data-name="Booking #'.$r['id'].'" title="Manage"><i class="fa fa-cog"></i> Manage</button>';
            $row[] = $actions;

            $stat = $r['status'] ?: 'open';
            $stclass = $stat=='New'?'primary':($stat=='Issued'?'success':($stat=='Awaiting Payment'?'danger':'info'));
            $stb = '<div class="badge bg-'.$stclass.'-subtle text-'.$stclass.' rounded-pill px-3 py-2">'.ucfirst($stat).'</div>';
            $row[] = $stb;
            
            $row[] = $pnr!=''?'<div class="badge bg-info-subtle text-secondary rounded-pill px-2 py-1" style="font-size: 0.8em;"><i class="fa fa-seat me-1"></i>'.ucfirst($pnr).'</div>': '-';
    ...
    if ($action === 'load') {
        ...
        $sql = "SELECT 
                ...
                bf.pnr,
                bf.amadeus_order_id,
                bf.travellers AS flight_travellers,
                ...
                bt.travel_date,
                bt.currency_symbol,
                bt.discount_id, bt.discount_code, bt.discount_name, bt.discount_type, bt.discount_value, bt.discount_amount, bt.original_amount, bt.final_amount,
                bt.travellers_count AS tour_travellers_count,
                bt.travellers_json AS tour_travellers,
                bt.tour_json

            FROM bookings i
            LEFT JOIN bookings_flights bf ON bf.booking_id = i.id
            LEFT JOIN bookings_tours bt   ON bt.booking_id = i.id
            LEFT JOIN channels ch ON i.channel_id = ch.id
            LEFT JOIN contact_types ct ON i.contact_type_id = ct.id
            LEFT JOIN bookings_types sc ON i.type_id = sc.id
            LEFT JOIN people p ON i.owner_id = p.id
        ...
        /* -------------------------
           Decode Travellers (JSON)
        -------------------------- */
        $travellers = [];

        if ((int)$row['type_id'] === 1 && !empty($row['flight_travellers'])) {
            // FLIGHT TRAVELLERS
            $decoded = json_decode($row['flight_travellers'], true);

        } elseif ((int)$row['type_id'] === 2 && !empty($row['tour_travellers'])) {
            // TOUR TRAVELLERS
            $decoded = json_decode($row['tour_travellers'], true);
        ...
        if (is_array($decoded)) {
            foreach ($decoded as $idx => $t) {
                $travellers[] = [
                    'name' => $t['name'] ?? '',
                    'email' => $t['email'] ?? '',
                    'phone' => $t['phone'] ?? '',
                    'dob' => !empty($t['dob']) ? date('d M Y', strtotime($t['dob'])) : '-',
                    'passport_number' => $t['passport_number'] ?? '-',
                    'passport_nationality' => strtoupper($t['passport_nationality'] ?? '-'),
                    'passport_expiry' => !empty($t['passport_expiry'])
                        ? date('d M Y', strtotime($t['passport_expiry']))
                        : '-'
                ];
            }
        }
        ...
        $row['traveller_count'] = 
            (int)$row['type_id'] === 2
                ? (int)($row['tour_travellers_count'] ?? count($travellers))
                : count($travellers);


        /* -------------------------
           Decode Tour Package JSON
        -------------------------- */
        if ((int)$row['type_id'] === 2 && !empty($row['tour_json'])) {

            $decodedTour = json_decode($row['tour_json'], true);

            if (is_array($decodedTour)) {

                $row['tour_package'] = [
                    'id'            => $decodedTour['id'] ?? null,
                    'name'          => $decodedTour['name'] ?? '',
                    'duration'      => $decodedTour['duration'] ?? '',
                    'base_cost'     => $decodedTour['total_cost'] ?? 0,
                    'sell_price'    => $decodedTour['sell_price'] ?? 0,
                    'margin_source' => $decodedTour['margin_source'] ?? '',
                    'components'    => $decodedTour['components'] ?? []
                ];

            } else {
                $row['tour_package'] = null;
            }
        ...
        // get package wordpress_url
        $wordpress_url = "";
        $sqlw = "SELECT wordpress_url FROM packages WHERE id=?";
        $stmt = $mysqli->prepare($sqlw);
        $stmt->bind_param('i',$row['package_id']);
        $stmt->bind_result($wordpress_url);
        $stmt->execute();
        $stmt->fetch();
        $stmt->close();
        $row['wordpress_url'] = $wordpress_url??"";
        ...
        if ((int)$row['type_id'] === 1) {
            $row['departure_dated'] = $row['travel_dated'] = $row['departure_date']!=''&&$row['departure_date']!='0000-00-00'?date("d M Y",strtotime($row['departure_date'])):'-';
            $row['return_dated'] = $row['return_date']!=''&&$row['return_date']!='0000-00-00'?date("d M Y",strtotime($row['return_date'])):'-';
        }
        /* -------------------------
           Normalize Tour Fields
        -------------------------- */
        if ((int)$row['type_id'] === 2) {
            $row['tour_name']        = $row['tour_name'] ?? '';
            $row['tour_duration']    = $row['tour_duration'] ?? '';
            $row['tour_price']       = $row['tour_price'] ?? 0;
            $row['travellers_count'] = $row['tour_travellers_count'] ?? $row['traveller_count'];
            $row['travel_dated'] = $row['travel_date']!=''&&$row['travel_date']!='0000-00-00'?date("d M Y",strtotime($row['travel_date'])):'-';
        }

        $row['tickets'] = $row['tickets'] ?? null;

        
        // return the row as-is (caller will use fields)
        json_exit(['success' => true, 'booking' => $row]);
    }
    ...
    if ($action === 'update') {
        ...
        foreach ($editable_map as $k => $col) {
            if (array_key_exists($k, $input)) {
                ...
                if($col=='assigned_to' && $val!='') {
                    ...
                    $geti = $db->get('bookings',array('id'=>$id),'assigned_to');
                    if($geti->assigned_to!=$val) {
                        $set_clauses[] = "`alert` = ?";
                        $params[] = '0';
                        $types .= 'd';
                    }
                    ...
                    $site->agent_log("Booking #$id is assigned to $pname");
                }
        ...
        $sql = "UPDATE bookings SET " . implode(', ', $set_clauses) . " WHERE id = ? LIMIT 1";
        $types .= 'i';
        $params[] = $id;
        ...
        $site->agent_log("Booking #$id is updated (summary: ".$input['subject'].")");
    ...
    if ($action === 'summary') {
        ...
        if ($type !== '') { $where .= " AND i.type_id=? "; $types.='i'; $params[]=$type; }
        ...
        if ($customer_id !== '') { $where .= " AND i.contact_entity_id=? "; $types.='i'; $params[]=$customer_id; }

        // total bookings
        $sqlTotal = "SELECT COUNT(*) AS c FROM bookings i $where";
        ...
        // open
        $sqlEnquiries = "SELECT COUNT(*) AS c FROM bookings i $where AND i.status = 'New'"; 
        ...
        // closed
        $sqlComplaints = "SELECT COUNT(*) AS c FROM bookings i $where AND i.status = 'Issued'";
        ...
        // working
        $sqlPending = "SELECT COUNT(*) AS c FROM bookings i $where AND i.status = 'Awaiting Payment'";
    ...
    if ($action === 'assigned_alerts') {
        ...
        $q = $mysqli->prepare("
            SELECT i.id,
                   ...
                   i.status,
                   i.contact_name,
                   c.name AS channel_name, sc.name as type_name
            FROM bookings i
            LEFT JOIN channels c ON c.id = i.channel_id
            LEFT JOIN bookings_types sc ON i.type_id = sc.id
            WHERE i.assigned_to = ? AND i.owner_id!=i.assigned_to
              AND IFNULL(i.alert, 0) = 0
        ...
        while ($row = $res->fetch_assoc()) {
            ...
            $row['timed'] = date("h:i A",strtotime($row['time']));
            $stat = $row['status'];
            $row['stclass'] = $stat=='New'?'primary':($stat=='Issued'?'success':($stat=='Awaiting Payment'?'danger':'info'));
            $data[] = $row;
        }
    ...
    if ($action === 'meeting_alerts') {

        $sql = "
            SELECT id, contact_name, meeting_date, meeting_time, meeting_timezone, status
            FROM bookings
            WHERE meeting_date >= CURDATE()
            AND (meeting_alert_on IS NULL OR meeting_alert_on < CURDATE())
    ...
    if ($action === 'mark_meeting_alert_read') {
    ...
    if ($action === "followup_list") {
        ...
        $booking = intval($_POST['id'] ?? 0);

        if ($booking <= 0) {
            echo json_encode(["status" => false, "msg" => "Invalid booking"]);
            exit;
        }
    ...
    if ($action === "followup_add") {
        ...
        $stmt = $mysqli->prepare("
            ...
            VALUES (?,?,?,?)
        ");
        $stmt->bind_param("isis", $booking, $note_text, $agent_id, $agent_name);
        $stmt->execute();
        ...
        $row = $mysqli->query("
            ...
            FROM bookings_followup
            WHERE id = $new_id
        ")->fetch_assoc();

        $uid = "";
        $getint = $db->get('[bookings=i]',array('i.id'=>$booking,'#join'=>"left join contact_types as ct on ct.id=i.contact_type_id"),'i.contact_type_id,ct.slug as contact_type_slug,i.contact_entity_id');
        if($getint) {
        ...
        $site->agent_log("Added booking followup for booking #$booking - $snippet", $uid, $utype,'notimeline');
    ...
    if ($action === 'documents_list') {
        ...
        if ($booking_id <= 0) {
            json_exit(['success' => false, 'error' => 'invalid_booking']);
        }
        ...
        $data = [];
        while ($row = $res->fetch_assoc()) {
            $row['file_url'] = 'uploads/bookings/' . $row['file_name'];
            $data[] = $row;
        }
        $stmt->close();
    ...
    if ($action === 'documents_add') {
        ...
        /* Upload directory */
        $uploadDir = __DIR__ . '/../../uploads/bookings/';
        if (!is_dir($uploadDir)) {
        ...
        $newName = 'booking_'.$booking_id.'_'.uniqid().'.'.$ext;
        ...
        $stmt = $mysqli->prepare("
            ...
            (booking_id, label, file_name, file_type, expiry_date, created_by, created_at)
            VALUES (?,?,?,?,?,?,NOW())
        ");
        $stmt->bind_param(
            "isssss",
        ...
        $site->agent_log("Document added to booking #$booking_id ($label)");
    ...
    if ($action === 'documents_delete') {
        ...
        /* Delete file */
        $filePath = __DIR__ . '/../../uploads/bookings/' . $doc['file_name'];
        if (is_file($filePath)) {
            @unlink($filePath);
        }

        /* Delete record */
        $stmt = $mysqli->prepare("DELETE FROM bookings_documents WHERE id = ?");
        $stmt->bind_param("i", $id);
        $stmt->execute();
        $stmt->close();

        $site->agent_log("Document deleted from booking #".$doc['booking_id']);

        json_exit(['success' => true]);
    }

    if($action=='payments_load'){

        $id = (int)$_GET['id'];

        $booking = $mysqli->query("
            SELECT id, payment_token, contact_entity_id, status
            FROM bookings
            WHERE id=$id
        ")->fetch_assoc();
        ...
        if(!$booking['payment_token']){
            ...
            if (isset($site) && method_exists($site, 'agent_log')) {
                ...
                $site->agent_log(
                    "Payment link generated for booking #$id",
                    $booking['contact_entity_id'],
                    "customer"
                );
        ...
        $payment_link = $protocol.$_SERVER['HTTP_HOST'].$root.($root!="/"?"/":"")."payment/?token=".$token;
        // $payment_linkx = $_SERVER['HTTP_HOST']." | ".$root." | "."payment/?token=".$token;
        // $payment_linkx = $protocol.$_SERVER['HTTP_HOST'].$root."payment/?token=".$token;
        ...
        if($r = $res->fetch_assoc()){
            ...
            $raw = json_decode($r['raw_response'],true);
            ...
            /* Card information */
            $cardScheme      = $source['scheme'] ?? '';
            $cardLast4       = $source['last4'] ?? '';
            $cardType        = $source['card_type'] ?? '';
            $cardCategory    = $source['card_category'] ?? '';
            $cardIssuer      = $source['issuer'] ?? '';
            $cardCountry     = $source['issuer_country'] ?? '';
            $cardProduct     = $source['product_type'] ?? '';
            $expiryMonth     = $source['expiry_month'] ?? '';
            $expiryYear      = $source['expiry_year'] ?? '';
            $cardHolder      = $source['name'] ?? '';

            /* Processing information */
            $acquirerTxn     = $processing['acquirer_transaction_id'] ?? '';
            $rrn             = $processing['retrieval_reference_number'] ?? '';
            ...
            $card = '';
            if($cardScheme && $cardLast4){
                $card = $cardScheme.' ****'.$cardLast4;
            }

            $cardExpiry = '';
            if($expiryMonth && $expiryYear){
                $cardExpiry = str_pad($expiryMonth,2,'0',STR_PAD_LEFT).'/'.$expiryYear;
            }
            ...
            $payment = [
                ...
                /* processing details */
                'acquirer_txn'     => $acquirerTxn,
                'rrn'              => $rrn,
                'scheme_id'        => $schemeId,

                /* customer */
                'cko_customer_id'  => $customerId,
                'cko_customer_email'=> $customerEmail,
                'cko_customer_name'=> $customerName
            ];
    ...
    if($action == 'send_payment_sms'){
        ...
        $sms = "Dear ".$name.", your payment link for booking #".$booking_id." is:\n".$link."\n\nPlease complete the payment.";


        /* SEND SMS */

        $sent = $site->send_sms_with_url($phone,$sms);
        ...
        if($sent){
            ...
            if (isset($site) && method_exists($site, 'agent_log')) {
                ...
                $site->agent_log(
                    "Payment SMS sent for booking #$booking_id to $phone and booking status changed",
                    $row['contact_entity_id'],
                    "customer"
                );
            ...
            if (isset($site) && method_exists($site, 'agent_log')) {
                ...
                $site->agent_log(
                    "Payment SMS FAILED for booking #$booking_id",
                    $row['contact_entity_id'],
                    "customer"
                );
    ...
    if($action == 'mark_awaiting_payment'){
        ...
        /* Only update if not already awaiting payment */
        ...
        if($row['status'] !== 'Awaiting Payment'){
            ...
            if (isset($site) && method_exists($site, 'agent_log')) {
                ...
                $site->agent_log(
                    "Payment link shared for booking #$booking_id (Awaiting Payment)",
                    $row['contact_entity_id'],
                    "customer"
                );
