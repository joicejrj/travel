...
$updated = $db->update('customers_reminders', ['id' => $reminder_id, 'customer_id' => $customer_id], $data);

if ($updated) {
    $getcontact = $db->get('customers',array('id'=>$customer_id),'name');
    $site->agent_log("Reminder updated to ".date("d M Y h:i A",strtotime($reminder_at)),$customer_id);
    $rtn['success'] = 1;
