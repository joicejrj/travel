...
switch ($action) {
  ...
  /* ==========================================================
     ...
     ========================================================== */
  case 'save':
    $recruiter_id = intval($_POST['supplier_id'] ?? 0);
    ...
    $stmt = $mysqli->prepare("
      INSERT INTO recruiters_logs (recruiter_id, name, notes, agent_id, type, visibility, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    ");
    $stmt->bind_param("ississs", $recruiter_id, $name, $notes, $agent_id, $type, $visibility, $datetime);
    $success = $stmt->execute();
    $stmt->close();

    if ($success) {
      // Log activity
      $getcontact = $db->get('recruiters', ['id' => $recruiter_id], 'name');
      $contact_name = $getcontact->name ?? '';
      $site->agent_log("Note added (Note: {$notes})",$recruiter_id,'recruiter');

      // Maintain daily_followup
      $today = date('Y-m-d');
      $status = '';
      $s = $mysqli->prepare("SELECT type FROM recruiters WHERE id = ? LIMIT 1");
      if ($s) {
        $s->bind_param("i", $recruiter_id);
        $s->execute();
      ...
      $q->bind_param("iis", $agent_id, $recruiter_id, $today);
      $q->execute();
      ...
      if ($q->fetch()) {
        ...
        $ins->bind_param("iissiii", $agent_id, $recruiter_id, $today, $status, $startcount, $rem_done, $note_done);
        $ins->execute();
        $ins->close();
      }
  ...
  case 'update':
    ...
    $stmt = $mysqli->prepare("UPDATE recruiters_logs SET notes=?, type=?, visibility=? WHERE id=? AND (agent_id=? OR visibility='Public')");
    $stmt->bind_param("sssii", $notes, $type, $visibility, $id, $agent_id);
    $success = $stmt->execute();
    $stmt->close();

    if($success) {
      $site->agent_log("Note updated (Note: {$notes})",$recruiter_id,'recruiter');
    }
  ...
  case 'delete':
    ...
    $stmt = $mysqli->prepare("DELETE FROM recruiters_logs WHERE id = ? AND agent_id = ?");
    $site->agent_log("Note deleted [$id]",$recruiter_id,'recruiter');
