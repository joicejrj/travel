                ...
                window.loadPaymentCategories = function(type, selectedCategory = null) {
                  ...
                  $(function () {
                    ...
                    $("#payment_status").on("change", function() {
                      ...
                      // updateChequeFields();
                      $("#payment_payment_method").trigger("change");
                    });
                    ...
                    window.updateStatusOptions = function () {
                        ...
                        const current = $("#payment_status").val();
                        const $status = $("#payment_status");

                        $status.empty(); // clear all
                        if(isNew && $("#payment_user_type").val() === "employee") {
                          $("#payment_type").val("Expense").trigger("change");
                          type = "Expense";
                        }
                        if (type === "Expense" && $("#payment_user_type").val() === "employee") {
                            $status.append(`<option value="Paid">Paid</option>`);
                            $status.append(`<option value="Partial Paid">Partial Paid</option>`);
                            // If previous status was not allowed → reset to Paid
                            if (current !== "Paid" && current !== "Partial Paid") {
                                $status.val("Paid");
                            } else {
                                $status.val(current);
                            }
