...
function showFareBreakup(orderJson){
    ...
    const price =
        data?.ReservationResponse?.Reservation?.Offer?.[0]?.Price;

    const breakdown =
        price?.PriceBreakdown?.[0]?.Amount;

    if(price && breakdown){

        const currency = breakdown?.CurrencyCode?.value || 'GBP';

        html += `
        <div class="mb-3">

            <div class="fw-semibold mb-2">
                <i class="fa fa-user me-1"></i> Adult (ADT)
            </div>
            ...
            <table class="table table-sm table-bordered align-middle">
                ...
                <tr>
                    <td>Base Fare</td>
                    <td class="text-end">
                        ${currency} ${Number(breakdown.Base).toFixed(2)}
                    </td>
                </tr>
        `;

        /* ---------- TAXES ---------- */
        if(breakdown?.Taxes?.Tax?.length){

            breakdown.Taxes.Tax.forEach(t => {
                html += `
                <tr>
                    <td>
                        Tax (${t.taxCode})
        ...
        html += `
            <tr class="fw-bold border-top">
                <td>Total Taxes</td>
                <td class="text-end">
                    ${currency} ${Number(price.TotalTaxes).toFixed(2)}
                </td>
            </tr>
            ...
            <tr class="fw-bold">
                ...
                <td class="text-end text-primary">
                    ${currency} ${Number(price.TotalPrice).toFixed(2)}
                </td>
            </tr>
        </table>
        </div>
        ...
        /* ---------- FILED AMOUNT ---------- */
        if(breakdown?.FiledAmount){

            html += `
            <div class="alert alert-light border small mb-0">
                <strong>Filed Amount:</strong>
                ${breakdown.FiledAmount.currencyCode}
                ${breakdown.FiledAmount.value}
            </div>`;
        }
