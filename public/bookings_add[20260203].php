...
// $pdfPath = generateBookingPDF($mysqli, 17);
// $pdfPath = generateBookingPDF($mysqli, 20);
...
if ($booking_id > 0) {
    ...
    $stmt = $mysqli->prepare("
        SELECT b.*,
               f.trip_type, f.origin, f.destination, f.departure_date, f.return_date, f.people_no, f.class, f.non_stop,
    ...
    <form id="detailForm" data-current-id="" novalidate>
      ...
      <div class="wizard-step" id="wiz-step-2" aria-hidden="true" style="display:none;">
          ...
          <div class="flight-bar card shadow-sm">
            <div class="card-body p-2">
              ...
              <div class="flight-tabs mb-2">
                <button type="button" class="<?=$is_existing&&$booking['type_id']=='1'&&$booking['trip_type']=='ONE_WAY'?'active':(!$is_existing?'active':'')?>" data-type="ONE_WAY">One Way</button>
                <button type="button" class="<?=$is_existing&&$booking['type_id']=='1'&&$booking['trip_type']=='ROUND_TRIP'?'active':''?>" data-type="ROUND_TRIP">Round Trip</button>
              </div>
        ...
        <script>
          ...
          <div id="tourDiv" class="border rounded-3 p-3 mt-3 bg-light" style="display:none;">
            ...
            <!-- Selected -->
            <div id="selectedTourWrap" class="d-none">
              <div class="d-flex justify-content-between align-items-center mb-2">
            ...
            <!-- Results -->
            <div id="tourResultsWrap">
              <h6 class="mb-3">Available Tour Packages</h6>
            ...
            <script>
              ...
              function loadTourPackages() {
                ...
                showPageLoader('Loading tour packages…');
            ...
            <script>
              function renderTourPackages() {
                ...
                if (!TOUR_PACKAGES.length) {
                  wrap.innerHTML =
                    `<div class="alert alert-warning">No tour packages found</div>`;
                  return;
                }
                  ...
                  card.innerHTML = `
                    <div>
                      ...
                      <div class="small mt-1">
                        Duration:
                        <span class="badge bg-light text-dark">${p.duration}</span>
                      </div>
                    </div>

                    <div>
                      <div class="price">£ ${p.amount}</div>
                      <div class="muted">Per package</div>
                    </div>
            ...
            <script>
              function selectTour(idx, btn) {
                ...
                document.getElementById('tourtrano').style.display = '';

                /* 4. Travellers */
                const count = parseInt(
                  document.getElementById('tourTravellersCount').value || '1',
                  10
                );
                renderTravellers(count);
            ...
            <script>
              document.getElementById('changeTourBtn')
                .addEventListener('click', function () {
                  ...
                  const card = selectedWrap.querySelector('.tour.card');
                  if (card) {
                    card.classList.remove('selected');
                    resultsWrap.prepend(card);
                  }
            ...
            <div class="mb-3" id="tourtrano" style="display: none;">
              <label class="form-label fw-semibold">No. of Travellers</label>
              <select id="tourTravellersCount"
                      class="form-select"
            ...
            <script>
              document.getElementById('tourTravellersCount')
              .addEventListener('change', function () {
                ...
                /* Autofill Traveller 1 (same logic as flight) */
                if (count >= 1) {
                  document.querySelector('.traveller-name[data-idx="1"]').value =
                    document.getElementById('ee-selected-customer-name').value || '';
