...
function generateBookingPDF(mysqli $mysqli, int $booking_id)
{
    ...
    /* Tour package snapshot */
    $tourPackage = [];

    if ($scenario_id === 2 && !empty($details['tour_json'])) {
        $decoded = json_decode($details['tour_json'], true);
        if (is_array($decoded)) {
            $tourPackage = $decoded;
        }
    }
    ...
    <table class="meta">
      ...
      <tr>
        ...
        <td class="value">'.$booking['subject'].'</td>
        <td class="label">Travel Date</td>
        <td class="value">'.($scenario_id === 1?($details['departure_date']!=''?date("d M Y",strtotime($details['departure_date'])):'-'):($details['travel_date']!=''?date("d M Y",strtotime($details['travel_date'])):'-')).'</td>
      </tr>
    </table>
    ';
    ...
    if ($scenario_id === 2) {
        ...
        <table class="meta">
          <tr>
            ...
            <td class="value">'.htmlspecialchars($details['tour_name']).'</td>
            <td class="label">Travellers</td>
            <td class="value">'.$details['travellers_count'].'</td>
          </tr>
          <tr>
            <td class="label">Duration</td>
            <td class="value">'.$details['tour_duration'].'</td>
            <td class="label">Package ID</td>
            <td class="value">'.$details['package_id'].'</td>
          </tr>
        </table>
        ';

        /* ---------- COMPONENTS ---------- */
        if (!empty($tourPackage['components'])) {
            ...
            $i = 1;
            foreach ($tourPackage['components'] as $c) {
                ...
                <tr class="row">
                  ...
                  <td class="right">'.($c['qty'] ?? 1).'</td>
                  <td class="right">'.$currency_symbol.number_format((float)($c['sell'] ?? 0), 2).'</td>
                </tr>';
            }
        ...
        /* ---------- PRICING ---------- */
        $baseCost  = $tourPackage['base_cost']  ?? 0;
        $sellPrice = $tourPackage['sell_price'] ?? $details['tour_price'];
        $sellPrice = $details['final_amount']>0?$details['final_amount']:$sellPrice;
        ...
        <table class="total-box">
          <tr>
            <td>Base Cost</td>
            <td class="right">'.$currency_symbol.number_format($baseCost, 2).'</td>
          </tr>
          <tr>
            <td>Charges</td>
            <td class="right">'.$currency_symbol.number_format(($tourPackage['sell_price']-$baseCost), 2).'</td>
          </tr>
          <tr>
            <td>Discount</td>
            <td class="right">'.$currency_symbol.number_format(($discount), 2).'</td>
          </tr>
          <tr>
            <td>Total Package Price</td>
            <td class="right">'.$currency_symbol.number_format($sellPrice, 2).'</td>
          </tr>
        </table>
        ';

        $amount = $currency_symbol . number_format($sellPrice, 2);
    }
