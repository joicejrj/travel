...
class TravelportAPI {
    ...
    public function searchFlights($params) {
        ...
        if (($params['trip_type'] ?? 'oneway') === 'roundtrip' && !empty($params['return_date'])) {
            $searchCriteria[] = [
                '@type'         => 'SearchCriteriaFlight',
        ...
        // Journey-based for round-trip
        if (($params['trip_type'] ?? 'oneway') === 'roundtrip') {
        ...
        return $this->apiRequest(EP_SEARCH, 'POST', $requestBody);
    }
    ...
    /**
     * Price selected flights using BuildFromProducts
     */
    ...
    public function initiateWorkbench($bookingId = null) {
        ...
        return $this->apiRequest(EP_WORKBENCH, 'POST', $body, $bookingId);
    }
     ...
     */
    public function retrieveReservation($pnr, $detailView = true) {
     ...
     */
    public function getTicketList($pnr) {
