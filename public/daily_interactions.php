...
<div class="container" style="padding:18px;">
  <a href="./?page=daily_interactions1" class="btn btn-sm btn-primary float-end"><i class="fa fa-plus"></i> New</a>
  ...
  <div class="page-filters">
    ...
    <div class="filter-field">
      ...
      <select id="filter-scenario" style="padding:8px;border-radius:6px;border:1px solid #d1d5db;">
        <option value="">All</option>
        <?php
        $rs = $mysqli->query("SELECT id,name FROM scenarios ORDER BY name");
        while ($r = $rs->fetch_assoc()) {
            echo '<option value="'.(int)$r['id'].'">'.htmlspecialchars($r['name']).'</option>';
        }
        $rs->free();
  ...
  <div class="row g-3 mb-3" id="summaryRow">
      <div class="col-md-3">
          <div class="card text-center shadow-sm">
              <div class="card-body">
                  <h5 class="card-title mb-1">Total Interactions</h5>
      ...
      <div class="col-md-3">
          <div class="card text-center shadow-sm">
              <div class="card-body">
                  <h5 class="card-title mb-1">Open</h5>
                  <h3 id="sum-open" class="text-success mb-0">—</h3>
              </div>
          </div>
      </div>
      ...
      <div class="col-md-3">
          <div class="card text-center shadow-sm">
              <div class="card-body">
                  <h5 class="card-title mb-1">Closed</h5>
                  <h3 id="sum-closed" class="text-danger mb-0">—</h3>
              </div>
          </div>
      </div>
      ...
      <div class="col-md-3">
          <div class="card text-center shadow-sm">
              <div class="card-body">
                  <h5 class="card-title mb-1">Working</h5>
                  <h3 id="sum-working" class="text-warning mb-0">—</h3>
              </div>
          </div>
      </div>
  </div>
  ...
  <table id="interactionsTable" class="display cell-border compact">
    <thead>
      <tr>
        <th>Date / Time</th>
        <th>Name</th>
  ...
  <div class="modal-dialog modal-lg modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-header">
        <h5 class="modal-title" id="viewTitle">Interaction</h5>
  ...
  <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content">
      ...
      <div class="modal-header p-0 border-bottom-0">
        ...
        <!-- Nav Tabs -->
        <ul class="nav nav-tabs w-100 px-3 pt-3" id="interactionTabs" role="tablist">
        ...
        <div class="tab-content p-3">
            ...
            <div class="modal-body p-0 d-flex flex-column" style="height:60vh;">
              <div>
                <h6 class="modal-title fw-semibold mb-0">
                  <small id="followup_interaction_title" class="text-dark fw-bold"></small>
                </h6>
                <small id="followup_interaction_note" class="text-muted"></small>
              </div>
              ...
              <div class="p-3" style="flex:0 0 auto;">
                <form id="add-followup-form" class="d-flex gap-2 align-items-start">
                  <input type="hidden" id="followup_interaction_id" name="interaction_id">
            ...
            <form id="editForm">
              ...
              <div class="row g-3">
                ...
                <div class="col-md-4 d-none" id="natureWrapper">
                  <label class="form-label">Nature of Interaction</label>
                  <input type="text"
                         id="edit-nature"
                         class="form-control"
                         placeholder="Enter nature of interaction">
                </div>
          ...
          <div class="tab-pane fade" id="tabDocs">
            ...
            <div id="addDocSection" class="border rounded p-3 mb-3 d-none bg-light">
              <form id="interactionDocForm" enctype="multipart/form-data">
                <input type="hidden" name="interaction_id" id="doc_interaction_id">
            ...
            <table class="table table-sm table-bordered align-middle mb-0">
              ...
              <tbody id="interaction-documents-body">
                <tr>
...
function openInteractionDocuments(interactionId) {
    currentInteractionId = interactionId;
    $('#doc_interaction_id').val(interactionId);
    $('#addDocSection').addClass('d-none');
    $('#selectedDocLabel').val('');
    loadDocumentLabels();
    loadInteractionDocuments();
    // new bootstrap.Modal('#interactionDocumentsModal').show();
}
...
/* Load documents */
function loadInteractionDocuments() {
    $('#interaction-documents-body').html(
        '<tr><td colspan="4" class="text-center text-muted py-3">Loading…</td></tr>'
    );

    $.getJSON(
        'public/ajax/daily_interactions_list.php',
        { action: 'documents_list', interaction_id: currentInteractionId },
        function (res) {
            ...
            $('#interaction-documents-body').html(rows);
        }
    );
}

/* Upload document */
$('#interactionDocForm').on('submit', function (e) {
    e.preventDefault();
    ...
    $.ajax({
        url: 'public/ajax/daily_interactions_list.php',
        type: 'POST',
        data: fd,
        processData: false,
        contentType: false,
...
$(document).on('click', '.btn-delete-doc', function () {
    ...
    $.post(
        ...
        function (res) {
            if (res.success) loadInteractionDocuments();
...
// (function(){
  ...
  $('#interactionsTable tbody').on('click', 'button', function(e){
    const btn = this;
    ...
    //   // open interaction in new tab/view
    //   window.open('./?page=interaction_view&id=' + encodeURIComponent(id), '_blank');
    //   return;
    // }
...
function loadSummaryBoxes() {
    $.getJSON('public/ajax/daily_interactions_list.php?action=summary', {
        date_range: $('#filter-daterange-raw').val(),
        channel: $('#filter-channel').val(),
        contact_type: $('#filter-contact-type').val(),
        scenario: $('#filter-scenario').val(),
        assigned_to: $('#filter-assigned').val()
    }, function(res){
        if (!res.success) return;

        $('#sum-total').text(res.total);
        $('#sum-open').text(res.open);
        $('#sum-closed').text(res.closed);
        $('#sum-working').text(res.working);
    });
}
...
//     $.getJSON('public/ajax/daily_interactions_list.php?action=load&id=' + id, function(res){
//         if (!res.success) return alert('Load failed');
//         const it = res.interaction;

//         $('#viewTitle').text('Interaction #' + it.id);

//         let html = `
//             <div><strong>When:</strong> ${it.dated} ${it.timed}</div>
//             <div><strong>Contact:</strong> ${it.contact_name ?? '—'}</div>
//             <div><strong>Channel:</strong> ${it.channel_name ?? '—'}</div>
//             <div><strong>Type:</strong> ${it.contact_type_name ?? '—'}</div>
//             <div><strong>Scenario:</strong> ${it.scenario_name ?? '—'}</div>
//             <hr>
//             <div><strong>Summary:</strong><br>${it.subject ?? ''}</div>
//             <div><strong>Notes:</strong><br>${it.notes ?? ''}</div>
//             <hr>
//             <div><strong>Phone:</strong> ${it.contact_phone ?? '—'}</div>
//             <div><strong>Email:</strong> ${it.contact_email ?? '—'}</div>
//             <div><strong>Assigned:</strong> ${it.owner_name ?? '—'}</div>
//         `;
...
function openViewModal(id,editurl="") {
    $.getJSON('public/ajax/daily_interactions_list.php?action=load&id=' + id, function(res){
        ...
        $('#viewTitle').text('View Interaction');
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-4">
                    ...
                    <div class="badge bg-warning-subtle text-warning rounded-pill px-3 py-2">
                        <i class="fa fa-lightbulb me-1"></i>${it.scenario_name ?? '—'}
                    </div>
                </div>
            </div>
...
document.getElementById('tab_openbtn').addEventListener('click', function () {
  ...
  function openEdit(id) {
      $.getJSON('public/ajax/daily_interactions_list.php?action=load&id=' + id, function(res){
          ...
          $('#editTitle').text('Edit Interaction');
  ...
  $('#editSave').on('click', function(){
      ...
      $.ajax({
          url: 'public/ajax/daily_interactions_list.php?action=update',
          method: 'POST',
          ...
          success: function(res){
              ...
              $('#interactionsTable').DataTable().ajax.reload(null, false);
          }
      });
  });
...
window.openFollowups = function(interactionId) {
    document.getElementById('followup_interaction_id').value = interactionId;
    ...
    // const modalEl = document.getElementById('interactionFollowupModal');
    // const modal = new bootstrap.Modal(modalEl);
    // modal.show();

    // header
    fetch('public/ajax/daily_interactions_list.php',{ method:'POST', body: new URLSearchParams({ action:'load', id: interactionId }) })
      .then(r=>r.json()).then(res=>{
        if(res.success && res.interaction) {
          document.getElementById('followup_interaction_title').textContent = res.interaction.subject || '';
          document.getElementById('followup_interaction_note').textContent = res.interaction.notes || '';
        }
      });

    // followups list
    fetch('public/ajax/daily_interactions_list.php',{ method:'POST', body: new URLSearchParams({ action:'followup_list', id: interactionId }) })
      .then(r=>r.json()).then(res=>{
        list.innerHTML = '';
  ...
  document.getElementById('add-followup-form').addEventListener('submit', function(e){
    e.preventDefault();
    const interactionId = document.getElementById('followup_interaction_id').value;
    ...
    fetch('public/ajax/daily_interactions_list.php',{ method:'POST', body: fd })
      .then(r=>r.json()).then(res=>{
    ...
    document.querySelectorAll('#interactionTabs button[data-bs-toggle="tab"]')
    .forEach(btn => {
      btn.addEventListener('shown.bs.tab', function (e) {

        const target = e.target.getAttribute('data-bs-target');
        const interactionId = document.getElementById('tab_iid').value;
        if (!interactionId) return;

        if (target === '#tabFollowup') openFollowups(interactionId);
        if (target === '#tabEdit')     openEdit(interactionId);
        if (target === '#tabDocs')     openInteractionDocuments(interactionId);
