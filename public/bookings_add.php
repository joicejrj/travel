...
if ($booking_id > 0) {
    ...
    $getu = $db->get('customers',array('id'=>$booking['contact_entity_id']),'name, phone, email, dob, gender');
}
    ...
    <form id="detailForm" data-current-id="" novalidate>
      ...
      <div class="wizard-step" id="wiz-step-2" aria-hidden="true" style="display:none;">
        ...
        <div id="flightDiv" class="border rounded-3 p-3 mt-3 bg-light" style="display:none;">
          ...
          <div class="flight-container">
            ...
            <div class="flight-bar card shadow-sm">
              <div class="card-body p-2">
                ...
                <input type="hidden" id="seat_total_price" name="seat_total_price">
                <input type="hidden" name="tripType" id="tripType" value="<?=$is_existing&&$booking['type_id']=='1'?$booking['trip_type']:'ONE_WAY'?>">

                <input type="hidden" id="selectedFlightsInput" name="selected_flights">
                ...
                <div class="flight-tabs mb-2 d-flex justify-content-between align-items-center">
                  ...
                  <div class="trip-tabs">
                    ...
                    <button type="button" class="<?=$is_existing&&$booking['type_id']=='1'&&$booking['trip_type']=='MULTI_CITY'?'active':'d-none1'?>" id="tpMultiCityTab" data-type="MULTI_CITY">
                        Multi City
                    </button>
                  </div>
                  <!-- RIGHT: Provider -->
                  <div class="provider-tabs">
                    <input type="hidden" name="provider" id="flightProvider"
                      value="<?=$is_existing ? ($booking['provider'] ?? 'TRAVELPORT') : 'TRAVELPORT'?>">
                    <button type="button"
                      class="provider-btn <?=($is_existing && ($booking['provider'] ?? '')=='AMADEUS')?'active':''?>"
                      data-provider="AMADEUS">
                      Amadeus
                    </button>
                    <button type="button"
                      class="provider-btn <?=(!$is_existing || ($booking['provider'] ?? 'TRAVELPORT')=='TRAVELPORT')?'active':''?>"
                ...
                <div class="flight-row">
                  ...
                  <div class="flight-field flightfrom">
                    <span>FROM</span>
                    <select id="origin" name="origin" value="<?=$is_existing&&$booking['type_id']=='1'&&$booking['origin']!=''?$booking['origin']:''?>" class="airport-select">
                  ...
                  <div class="flight-field flightto">
                    <span>TO</span>
                    <select id="destination" name="destination" value="<?=$is_existing&&$booking['type_id']=='1'&&$booking['destination']!=''?$booking['destination']:''?>" class="airport-select">
                    ...
                    <div id="multiCityFields" style="display:none" class="mt-2">
                      <div id="legsContainer"></div>

                      <div class="mt-2">
                        <button type="button" class="btn btn-sm btn-xs btn-primary" id="addLegBtn">
                          + Add Flight
                        </button>
                        <small class="text-muted ms-2" style="font-size: 0.8em;">Max 6 flights</small>
                      </div>
                    </div>

                  <div class="flight-field ndates date">
                    <span>DEPART</span>
                  ...
                  <div class="flight-field ndates date">
                    ...
                    <!-- AMADEUS PAX -->
                    <div class="flight-field small" id="paxAmadeus" style="<?=$is_existing&&$booking['type_id']=='1'&&$booking['provider']=='AMADEUS'?'':'display: none;'?>">
                      <span>PAX</span>
                    ...
                    <!-- TRAVELPORT PAX -->
                    <div id="paxTravelport" style="<?=$is_existing&&$booking['type_id']=='1'&&$booking['provider']=='AMADEUS'?'display: none;':''?> gap:6px;">
                  ...
                  <div class="flight-check">
                    ...
                    <div class="flight-row" id="tp_extra_fields" style="display:none">
                        <div class="flight-field small">
                          <span>Preferred Airline</span>
                          <select name="preferred_airline" class="form-control">
                            ...
                            <optgroup label="Asia">
                                ...
                                <option value="OZ">Asiana (OZ)</option>
                            ...
                            <optgroup label="Europe">
                                ...
                                <option value="OS">Austrian (OS)</option>
                        ...
                        <div class="flight-field">
                          <span>Max Results</span>
                            <select class="form-select" name="max_results">
                                ...
                                <option value="50" selected>50 offers</option>
                                <option value="100">100 offers</option>
                                <option value="200">200 offers</option>
                            </select>
                        </div>
                        <div class="flight-field small">
                          <span>Max Stops</span>
                          <select name="max_stops" class="form-control">
          ...
          .flight-field.flightfrommc, .flight-field.flighttomc {
            max-width: 14.4em;
          ...
          <script>
            ...
            let currentLeg = 1;
            let selectedFlights = {};
            let CURRENT_LEG_OFFERS = [];

            let SELECTED_FLIGHTS = {}; // {1:offer, 2:offer}
            let CURRENT_LEG = 1;
          ...
          document.addEventListener('DOMContentLoaded', function () {
            ...
            document.querySelectorAll('.trip-tabs button').forEach(btn => {
              btn.addEventListener('click', function () {
                ...
                if(this.dataset.type === 'MULTI_CITY'){
                ...
                $('.flightfrom, .flightto, .swap-btn').toggle(this.dataset.type !== 'MULTI_CITY');
          ...
          <div id="selectedFlightWrap"
            ...
            <div id="seatSummaryBox" class="seat-summary-box"></div>
          </div>

            <div id="mcHeader" class="mb-2 fw-bold text-primary"></div>
            <div id="multiCitySummary" class="mb-3"></div>
        ...
        .flight-footer{
          display:grid;
          grid-template-columns: 240px 1fr 160px 97px; /* 160px 1fr 160px 120px; */ 
          margin-top:8px;
          align-items:center;
        }
        ...
        <script>
            function renderTravellers(count){
              ...
              // 🔹 STEP 1: store existing values
              let old = [];
              wrap.querySelectorAll('.traveller-block').forEach((el,i)=>{
                old[i] = {
                  name: el.querySelector('.traveller-name')?.value || '',
                  email: el.querySelector('.traveller-email')?.value || '',
                  phone: el.querySelector('.traveller-phone')?.value || '',
                  dob: el.querySelector('.traveller-dob')?.value || '',
                  gender: el.querySelector('.traveller-gender')?.value || ''
                };
              });
              ...
              // 🔹 STEP 2: render + restore only available old data
              list.forEach((type,i)=>{

                const label = type==='ADT' ? 'Adult' : type==='CNN' ? 'Child' : 'Infant';
                  ...
                  <div class="card mb-3 traveller-block"
                    ...
                    <div class="card-body">
                      ...
                      <div class="row g-3">
                        ...
                        <div class="col-md-4">
                          <label class="form-label1">Phone</label>
                          <input type="text"
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="travellers[${i}][gender]" class="form-control traveller-gender" data-idx="${i+1}">
                        ...
                        <div class="col-md-3 d-none">
                            ...
                            <input type="text" name="travellers[${i}][passport_number]" class="form-control traveller-passport_number" data-idx="${i+1}" required>
                        </div>

                        <div class="col-md-2 d-none">
                            <label>Passport Expiry</label>
                            <input type="date" name="travellers[${i}][passport_expiry]" class="form-control traveller-passport_expiry" data-idx="${i+1}" required>
                        </div>

                        <div class="col-md-3 d-none">
                            <label>Nationality</label>
                            <select name="travellers[${i}][passport_nationality]" class="form-control select2-country traveller-passport_nationality" data-idx="${i+1}" required>
                ...
                // 🔹 restore values if exists (ONLY up to new count)
                if(old[i]){
                  const block = wrap.querySelectorAll('.traveller-block')[i];
                  block.querySelector('.traveller-name').value  = old[i].name;
                  block.querySelector('.traveller-email').value = old[i].email;
                  block.querySelector('.traveller-phone').value = old[i].phone;
                  block.querySelector('.traveller-dob').value   = old[i].dob;
                  block.querySelector('.traveller-gender').value= old[i].gender;
                }
        ...
        function renderMultiCityLeg() {
          const keys = Object.keys(multiCityData);
          if (!keys.length) {
            console.warn("No multi-city data");
            renderFlightResults([]);
            return;
          }
          if (!multiCityData[currentLeg]) {
            currentLeg = parseInt(keys[0]);
          }
          CURRENT_LEG_OFFERS = multiCityData[currentLeg] || [];
          if (!CURRENT_LEG_OFFERS.length) {
            console.warn("No offers for leg:", currentLeg);
            renderFlightResults([]);
            return;
          }
          const totalLegs = keys.length;

            tripType = (document.getElementById('tripType')?.value || '').toUpperCase();
            var provider = document.getElementById('flightProvider').value;
            isRoundTrip = provider=='TRAVELPORT' && (tripType === 'ROUND_TRIP');
            document.getElementById("fresultTitle").innerHTML =
              isRoundTrip
                ? (currentLeg === 1 ? "Select Outbound Flight" : "Select Return Flight")
                : `Select Flight ${currentLeg} of ${totalLegs}`;

          document.getElementById("fresultTitle").style.display = "block";
          renderFlightResults(CURRENT_LEG_OFFERS);
        }
          ...
          var rulesBtn = "";
          if(o.showfare) {
            rulesBtn = `<button type="button" class="btn btn-outline-info btn-sm btn-select-rules me-1 d-none" onclick="loadSeatMap('${o.offeringId}', '${o.frProductId}',${i})" title="View Seat Map"> <i class="fa fa-th"></i> Seats</button><button type="button" class="btn btn-outline-primary btn-sm btn-select-rules me-1"
          ...
          Rules
            ...
            var provider = document.getElementById('flightProvider').value;
            isRoundTrip = provider=='TRAVELPORT' && tripType === 'ROUND_TRIP';
          var selectFlightbtn = (isMultiCity || isRoundTrip)?`selectMultiCityFlight(${i})`:`selectFlight(${i},this)`;
          ...
          document.getElementById('SearchFlightsBtn').addEventListener('click', function () {
            ...
            isMultiCity = document.getElementById('tripType')?.value === 'MULTI_CITY';
            ...
            // alert(isMultiCity+" "+document.getElementById('tripType')?.value);
            ...
            // 🔥 Normalize trip type
            let tripType = (fd.get('tripType') || '').toUpperCase();
            fd.set('trip_type', tripType);

            var provider = document.getElementById('flightProvider').value;
            isRoundTrip = provider=='TRAVELPORT' && tripType === 'ROUND_TRIP';
            
            // 🔥 Travelport filters (ensure always present)
            fd.set('preferred_airline', fd.get('preferred_airline') || '');
            fd.set('max_results', fd.get('max_results') || '50');
            fd.set('max_stops', fd.get('max_stops') || '');

            // 🔥 MULTI-CITY handling
            if (tripType === 'MULTI_CITY') {
                let legs = [];
                const origins = fd.getAll('mc_origin[]');
                const destinations = fd.getAll('mc_destination[]');
                const dates = fd.getAll('mc_departure_date[]');

                for (let i = 0; i < origins.length; i++) {
                  if (!origins[i] || !destinations[i] || !dates[i]) continue;
                ...
                // remove raw arrays
                fd.delete('mc_origin[]');
                fd.delete('mc_destination[]');
                fd.delete('mc_departure_date[]');

                // append structured legs
                fd.append('legs', JSON.stringify(legs));
            }
            ...
            .then(json => {
              ...
              if (!json.success) {
                ...
                if (!json.offers || !json.offers.length) {
                  ...
                  document.getElementById("flight-topbar").style.display = "none";

                  document.getElementById("fresultTitle").innerHTML = "No flights found";
                  document.getElementById("fresultTitle").style.display = "block";
              ...
              FLIGHT_OFFERS = json.offers;
                ...
                if (isMultiCity || isRoundTrip) {
                  ...
                  selectedFlights = {};
                  currentLeg = 1;
                  json.offers.forEach(o => {
                    const seq = o.sequence || 1;
                    if (!multiCityData[seq]) multiCityData[seq] = [];
                    multiCityData[seq].push(o);
                  });
                  renderMultiCityLeg(); // 🔥 REQUIRED
                }
                else {
        ...
        <script>
          ...
          function selectFlight(idx, btn) {
            ...
            document.querySelector('.traveller-dob[data-idx="1"]').value =
              document.getElementById('ee-selected-customer-dob').value || '';

            document.querySelector('.traveller-gender[data-idx="1"]').value =
              document.getElementById('ee-selected-customer-gender').value || '';
            ...
            // ✅ include seat price if exists
            if(document.getElementById('flightProvider').value === "TRAVELPORT"){
                updateFinalPrice();
            }

          }

            function selectMultiCityFlight(index) {
              if (!CURRENT_LEG_OFFERS || !CURRENT_LEG_OFFERS[index]) {
                console.error("Invalid index", index);
                return;
              }

              const offer = CURRENT_LEG_OFFERS[index];

              // ✅ store correctly
              selectedFlights[currentLeg] = offer;

              const totalLegs = Object.keys(multiCityData).length;

              if (currentLeg < totalLegs) {

                currentLeg++; // move forward

                renderMultiCitySummary(); // update summary BEFORE next step
                renderMultiCityLeg();

              } else {

                renderMultiCitySummary();
                finalizeMultiCitySelection();
              }
            }

            function renderMultiCitySummary() {
              const container = document.getElementById('multiCitySummary');
              if (!container) return;
              ...
              Object.keys(selectedFlights)
                ...
                .forEach(leg => {
                  ...
                  const f = selectedFlights[leg];
                  if (!f) return;

                  const seg = f.itineraries?.[0]?.segments?.[0];

                  html += `
                    <div class="border p-2 mb-1 rounded d-flex justify-content-between">
                      <div>
                        <strong>Flight ${leg}</strong> :
                        ${seg.departure.iataCode} → ${seg.arrival.iataCode}
                        (${seg.departure.at})
                      </div>
                      <button class="btn btn-sm btn-outline-primary"
                        onclick="editMultiCityLeg(${leg})">
                        Change
                      </button>
                    </div>
                  `;
            ...
            function editMultiCityLeg(leg) {
              leg = parseInt(leg);
              currentLeg = leg;
              // remove future selections
              Object.keys(selectedFlights).forEach(k => {
                if (parseInt(k) >= leg) delete selectedFlights[k];
              });
              renderMultiCitySummary();
              renderMultiCityLeg();
            }

            function finalizeMultiCitySelection() {
              const finalFlights = Object.values(selectedFlights);
              ...
              document.getElementById('multiCitySummary').style.display = 'none';

              /* -------------------------
                 1. STORE COMBINED DATA
              -------------------------- */
              document.getElementById('selectedFlightsInput').value =
                JSON.stringify(finalFlights);
              ...
              finalFlights.forEach(f => {
                total = parseFloat(f.price?.grandTotal || 0); // remove +=
              });

              document.getElementById('selected_price').value = total.toFixed(2);

              /* -------------------------
                 2. SHOW FINAL UI (IMPORTANT)
              -------------------------- */
              document.getElementById('flightResultsWrap').style.display = 'none';
              document.getElementById('fresultTitle').style.display = 'none';
              ...
              -------------------------- */
                let html = '';
                finalFlights.forEach((f, i) => {
                  const seg = f.itineraries?.[0]?.segments?.[0];
                  const airline = seg.carrierCode || '';
                  const flightNo = seg.number || '';
                  const from = seg.departure?.iataCode || '';
                  const to   = seg.arrival?.iataCode || '';
                  const dt = formatDT(seg.departure?.at);
                  html += `
                    <div class="flight-card selected mb-2 d-flex justify-content-between align-items-center">
                      <div>
                        <strong>Flight ${i+1}</strong>
                      </div>
                      <div class="flex-grow-1 text-center">
                        <strong>${airline} ${flightNo}</strong>
                        &nbsp;&nbsp;
                        ${from} → ${to}
                        &nbsp;&nbsp;
                        ${dt}
              ...
              /* -------------------------
                 4. GENERATE TRAVELLERS (SAME AS NORMAL)
              -------------------------- */
              var provider = document.getElementById('flightProvider').value;
              ...
              if (provider === "AMADEUS") {
                adults = parseInt(document.querySelector('[name="adults"]').value || '1', 10);
              } else {
                const tp_adults   = parseInt(document.querySelector('[name="tp_adults"]')?.value || '1', 10);
                const tp_children = parseInt(document.querySelector('[name="tp_children"]')?.value || '0', 10);
                const tp_infants  = parseInt(document.querySelector('[name="tp_infants"]')?.value || '0', 10);
                adults = tp_adults + tp_children + tp_infants;
              }
              ...
              document.getElementById('travellerDiv')
            ...
            function resetFlightSearchState() {
              /* ---------- DATA RESET ---------- */
              selectedFlights = {};
              multiCityData = {};
              CURRENT_LEG_OFFERS = [];
              ...
              /* ---------- UI RESET ---------- */
              const summary = document.getElementById('multiCitySummary');
              if (summary) {
                summary.innerHTML = '';
                summary.style.display = '';
              }

              const selectedCard = document.getElementById('selectedFlightCard');
          ...
          <script>
            document.getElementById('changeFlightBtn')
              .addEventListener('click', function () {
                ...
                if (isMultiCity) {
                    renderMultiCityLeg(); // ALWAYS START FROM LEG 1
                }
            });
          </script>
          <script>
          ...
          <div id="tourDiv" class="border rounded-3 p-3 pt-0 bg-light" style="display:none;">
            ...
            <script>
              function selectTour(idx, btn) {
                ...
                const tDob = document.querySelector('.traveller-dob[data-idx="1"]');
                const tGender = document.querySelector('.traveller-gender[data-idx="1"]');
            ...
            <div id="travellerDiv" class="border rounded-3 p-3 mt-4" style="display:none">
              ...
              <div class="mt-3">
                ...
                <div id="seatPriceDisplay" class="text-muted small"></div>
                <!-- DISCOUNTS -->
            ...
            <script>
              $(document).ready(function() {
                ...
                get_incoming_calls();
                <?php
                    if(!(DB_HOST=='127.0.0.1' && DB_USER=='root')) {
                ?>
      ...
      res.items.forEach(c => {
        html += `
          <button type="button"
            ...
            <strong>${c.name} ${c.gender ? '• ' + c.gender : ''}</strong><br>
...
function selectContact(c) {
  ...
  block.querySelector('.traveller-gender').value   = c.gender || 'Male';
...
<input type="hidden" id="ee-selected-customer-dob" value="">
<input type="hidden" id="ee-selected-customer-gender" value="">
...
function ee_setEmployee(emp) {
  ...
  $('#ee-selected-customer-dob').val(emp.dob || '');
  $('#ee-selected-customer-gender').val(emp.gender || 'Male');
...
function saveDetail(){
  ...
  if(scenario_idform=="1") {
    ...
    const orderedFlights = Object.keys(selectedFlights)
      .sort((a,b)=>a-b)
      .map(k => selectedFlights[k]);

    if (orderedFlights.length > 0) {
      // 🔥 MULTICITY MODE
      fd.append('selected_flights', JSON.stringify(orderedFlights));
...
items.forEach(function(it){
    ...
    html += '<div class="ee-customer-item"'
         ...
         +   ' data-ee-dob="' + escapeHtml(it.dob || '') + '"'
         +   ' data-ee-gender="' + escapeHtml(it.gender || '') + '"'
...
$customerList.find('.ee-customer-item').off('click').on('click', function(){
    ...
    var emp = {
        ...
        dob: $(this).data('ee-dob'),
        gender: $(this).data('ee-gender'),
...
(function(){
  ...
  if(is_existing=='1') {
    ...
    document.getElementById('ee-selected-customer-dob').value = '<?=$is_existing&&$getu?$getu->dob:''?>';
    document.getElementById('ee-selected-customer-gender').value = '<?=$is_existing&&$getu?$getu->gender:''?>';
...
<!-- seat selection code starts -->
<style>
    #seatMapModal.modal-backdrop.show {
        opacity: unset !important;
    }
</style>
<style>
    ...
    .seat-available:hover{ transform:scale(1.12); }
    ...
    .seat-premium:hover{ transform:scale(1.12); }
    ...
    .seat-pane{ display:none; }
    .seat-pane.active{ display:block; }
...
<div class="modal modal-backdrop fade" id="seatMapModal">
  <div class="modal-dialog modal-lg modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <h5><i class="bi bi-grid-3x3"></i> Seat Map</h5>
        <button class="btn-close" data-bs-dismiss="modal"></button>
      </div>
      <div class="modal-body" id="seatMapBody"></div>
        <!-- ✅ NEW: sticky footer summary -->
        <div class="seat-modal-footer">
            <div id="seatPopupSummary" class="seat-popup-summary"></div>

            <button class="btn btn-primary btn-sm" id="seatConfirmBtn" onclick="confirmSeatSelection()" disabled>
                Confirm Seats
            </button>
        </div>
    </div>
...
<script>
let seatPricesGlobal = {}; // {seat: {price}}
let selectedSeats = {}; // flightIndex => seat
let seatMapFlights = [];
let currentSeatOfferIndex = null;
function loadSeatMap(offeringId, productId, offerIndex) {

    currentSeatOfferIndex = offerIndex;
    ...
    body.innerHTML = `
        <div class="text-center py-4">
            ...
            <p class="mt-2 text-muted">Loading seat map...</p>
        </div>
    `;

    new bootstrap.Modal(document.getElementById('seatMapModal')).show();

    fetch('public/ajax/seat_map.php?offering_id=' + offeringId + '&product_id=' + productId)
        .then(r => r.json())
        .then(data => {
            if (data.error) {
            ...
            renderSeatMap(data, body); // reuse your renderer
        })
        .catch(err => {
            body.innerHTML = `<div class="alert alert-danger">${err.message}</div>`;
        });
}
function renderSeatMap(data, container) {

    seatMapFlights = data.seatMaps.map(sm => sm.flight || {});

    if (!data.seatMaps || !data.seatMaps.length) {
        container.innerHTML = `<div class="text-center py-4 text-muted">No seat map available</div>`;
        return;
    }

    let html = '';

    // ✅ TOP FLIGHT TABS (compact UI)
    if (data.seatMaps.length > 1) {
        html += `<div class="seat-tabs">`;
        data.seatMaps.forEach((sm, idx) => {
            const f = sm.flight || {};
            html += `
                <button class="seat-tab ${idx===0?'active':''}" onclick="switchSeatTab(${idx})">
                    ${f.carrier || ''}${f.number || ''}
    ...
    html += `<div class="seat-tab-content">`;
    ...
    data.seatMaps.forEach((sm, idx) => {
        ...
        if(sm.seatPrices){
            Object.assign(seatPricesGlobal, sm.seatPrices);
        }

        // build seat map per flight (IMPORTANT FIX)
        const allSeats = {}, prices = {};
        sm.seats.available.forEach(s => allSeats[s] = 'available');
        sm.seats.premium.forEach(s => allSeats[s] = 'premium');
        sm.seats.occupied.forEach(s => { if (!allSeats[s]) allSeats[s] = 'occupied'; });
        if (sm.seatPrices) Object.assign(prices, sm.seatPrices);

        html += `<div class="seat-pane ${idx===0?'active':''}" id="seatPane${idx}">`;

        // header
        html += `
        <div class="text-center mb-2">
            <strong>${f.carrier || ''} ${f.number || ''}</strong>
        ...
        <div class="text-center small mb-2 text-muted">
            <span class="text-success">${sm.totalAvailable} free</span>
            ${sm.totalPremium ? `<span class="ms-2 text-primary">${sm.totalPremium} paid</span>` : ''}
            <span class="ms-2 text-secondary">${sm.totalOccupied} occupied</span>
        </div>`;

        const cabin = data.cabinLayouts?.[idx];

        if (cabin) {
            const cols = cabin.columns.map(c => c.letter);

            // aisle detection
            const aisles = new Set();
            cabin.columns.forEach((c,i)=>{
                if (c.position === 'A') aisles.add(i);
            });

            html += `<div class="cabin-box"><div class="seat-grid">`;

            // header
            html += `<div class="seat-row"><div class="seat-num-label"></div>`;
            cols.forEach((c,i)=>{
                html += `<div class="seat-cell seat-header">${c}</div>`;
                if (aisles.has(i)) html += `<div class="seat-aisle"></div>`;
            });
            html += `</div>`;

            // rows
            cabin.rows.forEach(row=>{
                html += `<div class="seat-row"><div class="seat-num-label">${row.label}</div>`;

                cols.forEach((c,i)=>{
                    const sn = row.label + c;
                    const status = allSeats[sn];

                    if (status && status !== 'occupied') {
                        html += `
                        <div class="seat-cell seat-${status}"
                            onclick="pickSeat(this,'${sn}',${idx})">${c}</div>`;
                    } else if (status === 'occupied') {
                        html += `<div class="seat-cell seat-occupied">×</div>`;
                    } else {
    ...
    html += `<div id="seatPickInfo" class="seat-summary-bar"></div>`;

    container.innerHTML = html;
}
function switchSeatTab(idx){
    document.querySelectorAll('.seat-tab').forEach((t,i)=>{
        t.classList.toggle('active', i===idx);
    });

    document.querySelectorAll('.seat-pane').forEach((p,i)=>{
        p.classList.toggle('active', i===idx);
    });
}
function pickSeat(el, seatNum, flightIdx){

    const pane = el.closest('.seat-pane');

    // remove only in this flight
    pane.querySelectorAll('.seat-selected')
        .forEach(s => s.classList.remove('seat-selected'));

    el.classList.add('seat-selected');

    selectedSeats[flightIdx] = seatNum;

    updateSeatPopupSummary(); // 🔥 NEW
    updateSeatSummaryUI();    // existing (outside modal)
}
function updateSeatPopupSummary(){

    const box = document.getElementById('seatPopupSummary');
    ...
    Object.entries(selectedSeats).forEach(([i, seat]) => {
        const f = seatMapFlights[i] || {};
    ...
    // enable confirm only if all flights selected
    btn.disabled = Object.keys(selectedSeats).length < seatMapFlights.length;
}
function updateSeatSummaryUI() {
    ...
    // render UI summary
    const box = document.getElementById('seatSummaryBox');
    if (!box) return;
    ...
    let html = `<div class="seat-summary-inner">`;
...
function confirmSeatSelection(){
    ...
    Object.values(selectedSeats).forEach(seat=>{
        if(seatPricesGlobal[seat]){
            totalSeat += parseFloat(seatPricesGlobal[seat].price || 0);
        }
    });

    document.getElementById('seat_total_price').value = totalSeat;

    // ✅ close modal
    bootstrap.Modal.getInstance(document.getElementById('seatMapModal')).hide();

    // ✅ AUTO SELECT FLIGHT (IMPORTANT)
    if (window.currentSeatOfferIndex !== undefined) {
        document.querySelectorAll('.flight-card')
            [window.currentSeatOfferIndex]
            ?.querySelector('.select-flight-btn')
            ?.click();
    }
...
function updateFinalPrice(){
    ...
    let base = parseFloat(document.getElementById('selected_price').value || 0);
    let seat = parseFloat(document.getElementById('seat_total_price').value || 0);
    ...
    // optional: show seat price separately
    let seatBox = document.getElementById('seatPriceDisplay');

    if(seatBox){
        seatBox.innerHTML = seat > 0
            ? `Seats: £ ${seat.toFixed(2)}`
            : '';
    }
}
</script>
...
<script>
function toggleTravelportUI(){
  const type = $('#detail-scenario').val();
  const provider = $('#flightProvider').val();

  resetFlightSearchState();

  if(type == '1' && provider === 'AMADEUS'){
    $('#tpMultiCityTab').addClass('d-none');
    $('#tp_extra_fields').hide();
    $(".flight-check").show();
    $('#multiCityFields').hide();
    $(".ndates").show();
    $('.flightfrom, .flightto, .swap-btn').show();

    if($('#tripType').val() === 'MULTI_CITY'){
      $('#tripType').val('ONE_WAY');
      document.querySelector('.trip-tabs button[data-type="ONE_WAY"]').click();
    }
  } else {
    $('#tpMultiCityTab').removeClass('d-none');
    $('#tp_extra_fields').show();
    $(".flight-check").hide();
    if($('#tripType').val() === 'MULTI_CITY'){
        $('.flightfrom, .flightto, .swap-btn').hide();
    }
  }
...
$('#flightProvider').on('change', toggleTravelportUI);
toggleTravelportUI();

// helpers / multicity utils section
function handleNewLegDefaults(){

  const origins = document.querySelectorAll('.multi-origin');
  const destinations = document.querySelectorAll('.multi-destination');
  const dates = document.querySelectorAll('.multi-date');

  const i = dates.length - 1;

  if (i > 0) {
    const prevDest = destinations[i-1]?.value || '';
    origins[i].value = prevDest;
  }

  if (i === 0) {
    const d = new Date();
    d.setDate(d.getDate() + 1);
    dates[i].value = formatDate(d);
  } else {
    const prevDate = dates[i-1]?.value;
    if (prevDate) {
      dates[i].value = addDays(prevDate, 3);
    }
  }
}
...
function renumberLegs(){
  ...
  const legs = document.querySelectorAll('.mc-leg'); // wrapper of each leg

  legs.forEach((leg, index) => {

    const title = leg.querySelector('.leg-title'); // where "Flight X" is shown

    if (title) {
      title.innerText = `Flight ${index + 1}`;
    }
...
document.addEventListener('change', function(e){
  ...
  if (!e.target.classList.contains('multi-date')) return;

  const dates = [...document.querySelectorAll('.multi-date')];

  const index = dates.indexOf(e.target);
  if (index === -1) return;
...
function addLeg(){
  ...
  let i = $('.mc-leg').length + 1;

  var optsmc = window.POPULAR_AIRPORTS.map(a => `<option value="${a.code}">${a.name}</option>`).join('');

  $('#legsContainer').append(`
    <div class="mc-leg flight-row align-items-center mb-2">

      <div class="me-2 small text-muted leg-title" style="width:60px">
        Flight ${i}
      </div>

      <div class="flight-field flightfrommc">
        <span>FROM</span>
        <select name="mc_origin[]" class="airport-select multi-origin">
          <option value="">City or Airport</option>
            ${optsmc}
        </select>
      </div>

      <button type="button" class="swap-btnmc swap-leg">⇄</button>

      <div class="flight-field flighttomc">
        <span>TO</span>
        <select name="mc_destination[]" class="airport-select multi-destination">
          <option value="">City or Airport</option>
            ${optsmc}
        </select>
      </div>
      ...
      <div class="flight-field date">
        <span>DEPART</span>
        <input type="date" name="mc_departure_date[]" class="form-control multi-date">
      </div>

      <div style="width:60px" class="text-end">
        ${i > 2 ? '<button type="button" class="btn btn-sm btn-danger removeLeg">✕</button>' : ''}
      </div>

    </div>
  `);

  handleNewLegDefaults();
  renumberLegs();

  // re-init select2 (same as normal fields)
  $('.airport-select').select2({
    theme: 'bootstrap-5',
    placeholder: 'City or Airport Code',
    // allowClear: true,
...
$(document).on('click','.swap-leg',function(){
  const row = $(this).closest('.mc-leg');
  const from = row.find('[name="mc_origin[]"]');
  const to = row.find('[name="mc_destination[]"]');

  let tmp = from.val();
  from.val(to.val()).trigger('change');
  to.val(tmp).trigger('change');
});
$(document).on('change', '[name="mc_destination[]"]', function(){
  let next = $(this).closest('.mc-leg').next();
  if(next.length){
    next.find('[name="mc_origin[]"]').val($(this).val()).trigger('change');
  }
});

function ensureTwoLegs(){
  if($('.mc-leg').length === 0){
    addLeg();
    addLeg();
  }
...
$(document).on('click','.removeLeg',function(){
  if($('.mc-leg').length <= 2){
    alert('Minimum 2 flights required');
    return;
  }
  $(this).closest('.mc-leg').remove();
  renumberLegs();
});
</script>