...
$status    = $_GET['status']         ?? 'unknown';   // success | failure | cancel
$reference = $_GET['ref']            ?? '';
$paymentId = $_GET['cko-payment-id'] ?? ($_GET['payment_id'] ?? '');

// Fetch & update session if we have a payment ID
$sessionRow  = null;
$paymentData = null;

if ($paymentId) {
    if ($reference) {
        // Synchronous payment: we have both ref and payment ID
        $paymentData = updateSessionStatus($reference, $paymentId);
        $sessionRow  = getSessionByReference($reference);
    } else {
        // Async redirect: only cko-payment-id in URL, look up session by payment ID
